...
typedef struct  
{
  ...
  int startPos;
  int startPosM2;
  int sao_offset[4];      //added by chenh
#endif	
}SAOBlkParam;
typedef struct
{
...
typedef struct
{
	int refBufIdx;//cms 当前帧在refbuf中的索引
  int picWidthInLCU;//<! frame row LCU number
  int picHeightInLCU;//<! frame col LCU  number
  int maxCUWidth;//<! LCU 的宽度 
  ...
  int current_df_yuv;              ////2016-6-1 提取整帧的去块滤波Bs参数,记录当前在滤波的色彩
