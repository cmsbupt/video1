/*
*****************************************************************************
* Contributors: Mingliang Chen,         Weiyao Lin,  Xiaozhen Zheng,
******************************************************************************
*/
...
int encode_for_P(int frame_num, Bitstream *bitstream)
{
  ...
  for (i = 1; i <= bitsBuff[0].para1; i++)
  {
    ...
    snprintf(str, TRACELENGTH, "PH: rect%03d_dx = %3d (org_x %3d, pred_x %3d)", i, bitsBuff[i].para1, bitsBuff[i].ptr->x, bitsBuff[i].ptr->x - bitsBuff[i].para1);
    len += se_v(str, bitsBuff[i].para1, bitstream);
    snprintf(str, TRACELENGTH, "PH: rect%03d_dy = %3d (org_y %3d, pred_y %3d)", i, bitsBuff[i].para2, bitsBuff[i].ptr->y, bitsBuff[i].ptr->y - bitsBuff[i].para2);
    len += se_v(str, bitsBuff[i].para2, bitstream);
    snprintf(str, TRACELENGTH, "PH: rect%03d_dw = %3d (org_w %3d, pred_w %3d)", i, bitsBuff[i].para3, bitsBuff[i].ptr->width, bitsBuff[i].ptr->width - bitsBuff[i].para3);
    len += se_v(str, bitsBuff[i].para3, bitstream);
    snprintf(str, TRACELENGTH, "PH: rect%03d_dh = %3d (org_h %3d, pred_h %3d)", i, bitsBuff[i].para4, bitsBuff[i].ptr->height, bitsBuff[i].ptr->height - bitsBuff[i].para4);
