...
int main ( int argc, char **argv )
{
  ...
  if(input->low_delay == 0)
  {
	  ...
	  {
		  cfg_dd_poc[i] = cfg_ref_all[i].poc + subgopOffset0;
		  ...
		  if ( i==input->jumpd_sub[2]+input->jumpd_sub[1]+input->jumpd_sub[0]+2 && input->jumpd_sub[0]!=0 &&input->jumpd_sub[1]!=0 &&input->jumpd_sub[2]!=0 )
	  ...
	  if (input->jumpd_sub[1]+input->jumpd_sub[0]+3>=(input->no_frames) && (input->no_frames)>input->jumpd_sub[0]+2 && input->jumpd_sub[1]!=0 )  //less than sub[1]
	  {
		  ...
		  picture_reorder_delay=max(tmp_pdd,picture_reorder_delay);
	  }

	  if (input->jumpd_sub[2]+input->jumpd_sub[1]+input->jumpd_sub[0]+4>=(input->no_frames) && (input->no_frames)>input->jumpd_sub[1]+input->jumpd_sub[0]+3 
	  ...
	  if ( input->jumpd_sub[3]+input->jumpd_sub[2]+input->jumpd_sub[1]+input->jumpd_sub[0]+5>=(input->no_frames) && (input->no_frames) >input->jumpd_sub[2]+input->jumpd_sub[1]+input->jumpd_sub[0]+4
	  ...
	  if (input->no_frames>gop_size_all)
	  {
		  ...
		  {
			  tmp_delta_dd = cfg_ref_all[j].poc + subgopOffset1 - (j + 1);
			  if (tmp_delta_dd < delta_dd)                
			  {                                          
				  delta_dd = tmp_delta_dd;
			  }
			  ...
			  if ( j==input->jumpd_sub[2]+input->jumpd_sub[1]+input->jumpd_sub[0]+2 && input->jumpd_sub[0]!=0 &&input->jumpd_sub[1]!=0 &&input->jumpd_sub[2]!=0 )
  ...
  AQPStepLength = gop_size_all;
  {
	  ...
	  for(i=0; i<AQPStepLength; i++)
	  {
		  AQPoffset[i]=cfg_ref_all[i].qp_offset;
  ...
  if (input->EncControl!=0)
  {
    ...
    memset(pRC,0,sizeof(RateControl));
    {
      ...
      *ch = '\0';
      ...
      Init_RateControl(pRC,input->EncControl,input->useDQP,input->no_frames,input->intra_period, input->TargetBitRate,img->framerate,IniQP,input->img_width,input->img_height,ch);
    }
  }
#endif
  ...
  switch(img->type)
  {
  case INTRA_IMG: stat->bit_ctr_0 += 32; break;
  case B_IMG:     stat->bit_ctr_B += 32; break;
  default:		stat->bit_ctr_P += 32; break;
  }
...
#else
		fprintf(stdout, " Duplicated GB bits                : %d \n", stat->bit_ctr_dup_gb);
#endif
		{
			double bitrate_without_dup_gb = (((double)total_bits - stat->bit_ctr_dup_gb)*frame_rate) / ((double)(input->no_frames));
...
//	  fprintf ( logtable, "%.3f", (pRC->HighestBufferSizeBpp-pRC->LowestBufferSizeBpp)/pRC->TargetBitPerPixel );
...
#if INTERLACE_CODING
  if (input->InterlaceCodingOption == 3)
    fprintf ( p_stat, " Bit rate (kbit/s)  @ %5.2f Hz     : %5.2f\n", frame_rate, 2 * stat->bitrate / 1000 );
  else
#endif
  fprintf ( p_stat, " Bit rate (kbit/s)  @ %5.2f Hz     : %5.2f\n", frame_rate, stat->bitrate / 1000 );
  ...
  if ( ( p_log = fopen ( "log.dat", "r" ) ) == 0 )          // check if file exist
  {
    ...
    else                                            // Create header for new log file
    {
		...
		fprintf ( p_log, " ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- \n" );
		fprintf ( p_log, "|Date |Time | Sequence                                   |#Img|Q1|Qn|  Format |Hada|S R|Ref|Freq|IP|SNRY 1|SNRU 1|SNRV 1|SNRY N|SNRU N|SNRV N| #Bitr P | #Bitr B |Bit Rate| Ecode T |\n" );
		fprintf ( p_log, " ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- \n" );
#endif
	}
  }
  ...
  fprintf ( p_log, "%4d|", input->no_frames );
  ...
  fprintf ( p_log, "%2d|", input->intra_period );
  ...
  if ( input->successive_Bframe != 0 )
  {
	  ...
	  if (input->InterlaceCodingOption == 3)
	  {
		  fprintf ( p_log, "%9.0f|", stat->bitrate_P );
		  fprintf ( p_log, "%9.0f|", stat->bitrate_B );
	  }
	  else
	  {
		  fprintf ( p_log, "%9.0f|", stat->bitrate_P );
  ...
  {
	  fprintf ( p_log, "%9.0f|", stat->bitrate );
	  fprintf ( p_log, "%9.0f|", 0.0 );
  }
  fprintf ( p_log, "%8.2f|", stat->bitrate / 1000 );
  fprintf ( p_log, "%9.2f|\n", tot_time * 0.001 );
#endif
  fclose ( p_log );
}
