...
void GetUserDefWQM(char* pchFile,int sizeId,int *src)
{
	...
	{
		snprintf (errortext, ET_SIZE, "Can't open file %s.\n %s.\n",pchFile);
		error (errortext, 303);
	}
	...
	do 
	{
		...
		{
			snprintf (errortext, ET_SIZE, "Error: can't read Matrix %s.\n",WQMType[sizeId]);
			error (errortext, 304);
		}
	...
	for(y=0; y<uiWqMSize; y++)
		for(x=0; x<uiWqMSize; x++)
		{
			...
			if((coef==0)||coef>255) 
			{
				snprintf (errortext, ET_SIZE, "QM coefficients %d is not in the range of [1, 255].\n",coef);
				error (errortext, 305);
			}
			else
			{
			...
			for(i=0; i<uiBlockSize; i++)
			{
				...
				else
				{
					if((uiMatrixId==0)||(uiMatrixId==1))
						LevelScaleNxN[1][j][i]    = (int)((float)(ScaleM[j&3][i&3]<<WQ_FLATBASE_INBIT)/cur_wq_matrix[uiMatrixId][j*uiBlockSize+i]);
#if AWQ_LARGE_BLOCK_ENABLE
#if AWQ_LARGE_BLOCK_EXT_MAPPING
    				else if (uiMatrixId==2)
						LevelScaleNxN[1][j][i]    = (int)((float)(ScaleM[j&3][i&3]<<WQ_FLATBASE_INBIT)/cur_wq_matrix[uiMatrixId][(j>>1)*(uiBlockSize>>1)+(i>>1)]);
					else if (uiMatrixId==3)
						LevelScaleNxN[1][j][i]    = (int)((float)(ScaleM[j&3][i&3]<<WQ_FLATBASE_INBIT)/cur_wq_matrix[uiMatrixId][(j>>2)*(uiBlockSize>>2)+(i>>2)]);
#endif
#else
				...
				{
					if((uiMatrixId==0)||(uiMatrixId==1))
						LevelScaleNxN[0][j][i]    = (int)((float)(ScaleM[j&3][i&3]<<WQ_FLATBASE_INBIT)/cur_wq_matrix[uiMatrixId][j*uiBlockSize+i]);
#if AWQ_LARGE_BLOCK_ENABLE
#if AWQ_LARGE_BLOCK_EXT_MAPPING
					else if (uiMatrixId==2)
						LevelScaleNxN[0][j][i]    = (int)((float)(ScaleM[j&3][i&3]<<WQ_FLATBASE_INBIT)/cur_wq_matrix[uiMatrixId][(j>>1)*(uiBlockSize>>1)+(i>>1)]);
					else if (uiMatrixId==3)
						LevelScaleNxN[0][j][i]    = (int)((float)(ScaleM[j&3][i&3]<<WQ_FLATBASE_INBIT)/cur_wq_matrix[uiMatrixId][(j>>2)*(uiBlockSize>>2)+(i>>2)]);
#endif
#else
					else if ((uiMatrixId==2)||(uiMatrixId==3))
						LevelScaleNxN[0][j][i]    = ScaleM[j&3][i&3];
...
void GetUserDefParam(char* str_param,int mode)
{
   ...
   {
	   snprintf (errortext, ET_SIZE, "Cannot read the weight parameters in configuration file %s.\n",str_param);
       error (errortext, 301);
   }
   ...
   if(num!=PARAM_NUM)
   {
	   snprintf (errortext, ET_SIZE, "Not all of the weight parameters is loaded in configuration file %s.\n",str_param);
       error (errortext, 302);
   }
