...
extern  int check_mv_range ( unsigned int uiBitSize, int mv_x, int mv_y, int pix_x, int pix_y, int blocktype ); // mv_range, 20071009
...
void start_codingUnit()
{
  ...
  for ( row = 0; row < num_of_orgMB_in_row; row++ )
  {
    ...
    for ( col = 0; col < num_of_orgMB_in_col; col++, pos++ ) //Liwr 0915
    {
      ...
      else if ( currMB->slice_nr == img->mb_data[img->current_mb_nr - 1].slice_nr )  // current MB belongs to the same slice as the last MB
      {
        currMB->bitcounter[BITS_HEADER] = 0;
      }
      ...
      if ( input->usefme )
      {
        ...
        if ( img->type != B_IMG )
        {
          for ( j = 0; j < BLOCK_MULTIPLE; j++ )
          {
            for ( i = 0; i < BLOCK_MULTIPLE; i++ )
            {
              all_mincost[ mb_pos_y + j][mb_pos_x + i][0][0][3] = img->tmp_mv[mb_pos_y + j][mb_pos_x + i][0];
              all_mincost[ mb_pos_y + j][mb_pos_x + i][0][0][4] = img->tmp_mv[mb_pos_y + j][mb_pos_x + i][1];
            }
          }
        }
        else
        {
          for ( j = 0; j < BLOCK_MULTIPLE; j++ )
          {
            for ( i = 0; i < BLOCK_MULTIPLE; i++ )
            {
              ...
              all_mincost[ mb_pos_y + j][ mb_pos_x + i][0][0][3] = img->fw_mv[mb_pos_y + j][mb_pos_x + i][0];
              all_mincost[ mb_pos_y + j][ mb_pos_x + i][0][0][4] = img->fw_mv[mb_pos_y + j][mb_pos_x + i][1];
              all_bwmincost[ mb_pos_y + j][ mb_pos_x + i][0][0][3] = img->bw_mv[mb_pos_y + j][mb_pos_x + i][0];
              all_bwmincost[ mb_pos_y + j][ mb_pos_x + i][0][0][4] = img->bw_mv[mb_pos_y + j][mb_pos_x + i][1];
            }
          }
        }
      }
...
void Init_Curr_codingUnit ( codingUnit *currMB, unsigned int uiBitSize, unsigned int PositionInPic ) //,int     **ipredmodes)
{
  int i, j, k, l;
...
#endif
{
  ...
  if ( !input->slice_set_enable ) //added by mz, 2008.04
  {
    if ( /*input->slice_row_nr &&*/ ( img->coded_mb_nr != img->PicSizeInMbs ) )
    {
      ...
      {
        pos_next = img->current_mb_nr + ( ( 1 << ( input->g_uiMaxSizeInBit - MIN_CU_SIZE_IN_BIT ) ) - 1 ) * img->PicWidthInMbs + num_of_orgMB_in_col;
      }
      else
      {
...
#else
	  if ( img->coded_mb_nr % img->mb_no_currSliceLastMB == 0 )
#endif
      {
...
//        img->mb_no_currSliceLastMB =  min ( img->mb_no_currSliceLastMB + slice_mb, img->PicSizeInMbs - 1 ) ;
      ...
      else
      {
        for ( j = 0; j < num_of_orgMB_in_row; j++ )
        {
          ...
          {
            img->mb_data[pos].slice_nr = img->mb_data[img->current_mb_nr].slice_nr;  // last MB of slice, begin next slice   by jlzheng  6.30
          }
        }
      }
    }
...
void OneComponentLumaPrediction8x8 ( unsigned int uiBitSize, int*   mpred, int    pic_pix_x, int    pic_pix_y, int PU_size_x, int PU_size_y, int*   mv, int    ref ) // <--  reference frame (0.. / -1:backward)

{
...
#else
  if ( img->type == F_IMG && input->b_dmh_enabled )
#endif
  {
    dmh_mode = mv[2];
  }
  ...
  for ( j = 0; j < PU_size_y; j++ )
  {
...

    {
      ...
      {
        *mpred++ = ( get_pel ( ref_pic, j0 + pix_add * j + first_y, i0 + pix_add * i + first_x) + get_pel ( ref_pic, j0 + pix_add * j + second_y, i0 + pix_add * i + second_x) + 1 ) / 2;
      }
    }
  }
}
...
#if WEIGHTED_SKIP
void LumaPrediction8x8 ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int  block_x, int  block_y, int  fw_mode, int  bw_mode, int  fw_ref, int  bw_ref, MCParam* pMCParam )
#else
void LumaPrediction8x8 ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int  block_x, int  block_y, int  fw_mode, int  bw_mode, int  fw_ref, int  bw_ref )
#endif
{
  ...
  int  *****fmv_array = ( fw_mode < 0 ) ? img->allBidFwMv : ( fw_mode > 0  && bw_mode > 0) ? img->allSymMv : ( fw_mode > 0  && bw_mode < 0 ) ? img->allDualFstMv : img->allFwMv;
  int  *****bmv_array = ( fw_mode < 0 ) ? img->allBidBwMv : ( fw_mode > 0  && bw_mode < 0 ) ? img->allDualSndMv : img->allBwMv;
  ...
  for ( i = 0; i < 4; i++ )
  {
    delta_P[i] = ( 2 * ( picture_distance - img->imgtr_fwRefDistance[i] ) + 512 ) % 512;
#if AVS2_SCENE_MV
	if (i == img->nb_references - 1 && background_reference_enable){
		delta_P[i] = 1;
	}
#endif
  }
  ...
  if ( fw_mode || ( direct && ( fw_ref != -1 ) ) || skipped )
  {
    ...
    if(pMCParam->dir != 0)
    {
      ...
      {
        OneComponentLumaPrediction8x8 ( uiBitSize, fw_pred, pic_pix_x, pic_pix_y, step_h, step_v, img->tmp_fstPSkipMv[pMCParam->dir], fw_ref );
      }
      else
      {
        OneComponentLumaPrediction8x8 ( uiBitSize, fw_pred, pic_pix_x, pic_pix_y, step_h, step_v, img->tmp_fwBSkipMv[pMCParam->dir], ( direct ? 0 : fw_ref ) );
      }
    }
    else
    {
      OneComponentLumaPrediction8x8 ( uiBitSize, fw_pred, pic_pix_x, pic_pix_y, step_h, step_v, fmv_array [by][bx][fw_ref][fw_mode], ( direct ? 0 : fw_ref ) ); //qyu 4x4->8x8
    }
    ...
    if ( img->type == F_IMG && num_skipmode && fw_mode == 0 )
    {
...
#if MV_SCALING_UNIFY
      mv_wgt[0] = ( delta_P[num_skipmode] * fmv_array[by] [bx][fw_ref][fw_mode][0] * ( MULTI / delta_P[0] ) + HALF_MULTI ) >> OFFSET ;
      mv_wgt[1] = ( delta_P[num_skipmode] * fmv_array[by] [bx][fw_ref][fw_mode][1] * ( MULTI / delta_P[0] ) + HALF_MULTI ) >> OFFSET;
#else
      mv_wgt[0] = ( delta_P[num_skipmode] * fmv_array[by] [bx][fw_ref][fw_mode][0] * ( 16384 / delta_P[0] ) + 8192 ) >> 14 ;
      mv_wgt[1] = ( delta_P[num_skipmode] * fmv_array[by] [bx][fw_ref][fw_mode][1] * ( 16384 / delta_P[0] ) + 8192 ) >> 14;
#endif
      ...
      if (img->is_field_sequence)
      {
        ...
        mv_wgt[1] =( ( delta_P[num_skipmode] * (fmv_array[by] [bx][fw_ref][fw_mode][1]+delta) * ( 16384 / delta_P[0] ) + 8192 ) >> 14 ) - delta2;
      }
#endif
      mv_wgt[2] = 0;
    ...
    if(img->type == F_IMG && input->dhp_enabled && dual )
    {
      ...
      OneComponentLumaPrediction8x8 ( uiBitSize, fw_pred_wgt, pic_pix_x, pic_pix_y, step_h, step_v, bmv_array [by][bx][fw_ref][bw_mode], bw_ref );
    ...
    {
      OneComponentLumaPrediction8x8 ( uiBitSize, fw_pred_wgt, pic_pix_x, pic_pix_y, step_h, step_v, img->tmp_sndPSkipMv[pMCParam->dir], bw_ref );
  ...
  if ( img->type == B_IMG && ( bw_mode || ( direct && ( bw_ref != -1 ) ) ) )
  {
    ...
    delta_PB = 2 * ( picture_distance - img->imgtr_fwRefDistance[0] );
    TRp  = ( TRp + 512 ) % 512;
    ...
    {
#if MV_SCALING_UNIFY
      mv[0] = - ( ( fmv_array[by][bx][fw_ref][fw_mode][0] * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET );
      mv[1] = - ( ( fmv_array[by][bx][fw_ref][fw_mode][1] * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET );
#else
      mv[0] = - ( ( fmv_array[by][bx][fw_ref][fw_mode][0] * DistanceIndexBw * ( 512 / DistanceIndexFw ) + 256 ) >> 9 );
      mv[1] = - ( ( fmv_array[by][bx][fw_ref][fw_mode][1] * DistanceIndexBw * ( 512 / DistanceIndexFw ) + 256 ) >> 9 );
#endif
      ...
      if (img->is_field_sequence )
      {
        ...
        getDeltas( &delta, &delta2, oriPOC, oriRefPOC, scaledPOC, scaledRefPOC);
        mv[1] = - ( ( (fmv_array[by][bx][fw_ref][fw_mode][1] + delta) * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET ) -delta2;
      }
#endif
    }
#if m3469
    ...
    if ( fw_mode && bw_mode )
    {
      ...
      if(pMCParam->dir != 0)
      {
        OneComponentLumaPrediction8x8 ( uiBitSize, bw_pred, pic_pix_x, pic_pix_y, step_h, step_v, img->tmp_bwBSkipMv[pMCParam->dir], ( -1 - bw_ref ) );
      }
      else
      {
    ...
    else
    {
      ...
      {
        OneComponentLumaPrediction8x8 ( uiBitSize, bw_pred, pic_pix_x, pic_pix_y, step_h, step_v, bmv_array[by][bx][bw_ref][bw_mode], ( -1 - bw_ref ) );
      }
    }
  }
...
#else
void LumaResidualCoding8x8_PRED ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int *cbp, int *cbp_blk, int block8x8, int fw_mode, int bw_mode, int fw_refframe, int bw_refframe, int **curr_blk )
#endif
{
...
#if WEIGHTED_SKIP
  LumaPrediction8x8 ( currMB, uiBitSize, uiPositionInPic, mb_x, mb_y, fw_mode, bw_mode, fw_refframe, bw_refframe, pMCParam );
#else
...
int LumaResidualCoding8x8_TRANS ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int *cbp, int *cbp_blk, int block8x8, int fw_mode, int bw_mode, int fw_refframe, int bw_refframe, int **curr_blk )
{
  int    i, j;
  int    coeff_cost = 0;
  int    iStartX,iStartY;
...
#endif
  {
    ...
    if  (( currMB->cuType == PSKIPDIRECT || currMB->cuType == P2NX2N || currMB->cuType == PNXN || currMB->trans_size == 0 || IS_INTRA(currMB)) || (input->useNSQT == 0)  || currMB->ui_MbBitSize == B8X8_IN_BIT )
    {
#if EXTEND_BD
			transform_B8 ( curr_blk, uiBitSize, currMB, 0, input->b_secT_enabled, input->sample_bit_depth );
#else
			transform_B8 ( curr_blk, uiBitSize, currMB, 0, input->b_secT_enabled );
#endif
    }
    else
    {
...
#else
			transform_NSQT ( curr_blk, uiBitSize, currMB, 0, input->b_secT_enabled );
#endif
    }
    g_block8x8     = block8x8;
	...
	coeff_cost = inverse_quantization(currMB->qp - MIN_QP, 0, block8x8, curr_blk, scrFlag, cbp,  uiBitSize, uiPositionInPic,currMB,0);
	...
	coeff_cost = inverse_quantization( img->qp - MIN_QP, 0, block8x8, curr_blk, scrFlag, cbp,  uiBitSize, uiPositionInPic,currMB, 0);
	inverse_transform( block8x8, curr_blk, uiBitSize, uiPositionInPic,currMB, 0);
  ...
  if( coeff_cost <= _LUMA_COEFF_COST_ )
  {
    ...
    for ( j = iStartY; j < iSizeY + iStartY; j++ )
    {
      ...
      {
        imgY[pix_y + j][pix_x + i] = img->predBlock[j][i];
      }
    }
...
#if WEIGHTED_SKIP
int LumaResidualCoding8x8 ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int *cbp, int *cbp_blk, int block8x8, int fw_mode, int bw_mode, int fw_refframe, int bw_refframe, MCParam* pMCParam )
#else
int LumaResidualCoding8x8 ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int *cbp, int *cbp_blk, int block8x8, int fw_mode, int bw_mode, int fw_refframe, int bw_refframe )
#endif
{
  int    pic_pix_y, pic_pix_x, i, j;
  int    coeff_cost = 0;
  ...
  *fw_mode = *bw_mode = *fw_ref = *bw_ref = -1;
  ...
  if ( img->type != B_IMG )
  {
	  if ( currMB->b8pdir[b8] == DUAL )
	  {
		  ...
		  *bw_ref = p_snd_refFrArr[block_8y + j][block_8x + i];
...
#else
void LumaResidualCoding ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic )
#endif
{
  int i, j, block8x8;
  ...
  if ( currMB->trans_size == 1 )
  {
    ...
    {
      SetModesAndRefframe ( currMB, uiBitSize, uiPositionInPic, block8x8, &fw_mode[block8x8], &bw_mode[block8x8], &refframe, &bw_ref ); //qyu -1
#if WEIGHTED_SKIP
      LumaResidualCoding8x8_PRED( currMB, uiBitSize - 1, uiPositionInPic, & ( currMB->cbp ), & ( currMB->cbp_blk ), block8x8, fw_mode[block8x8], bw_mode[block8x8], refframe, bw_ref, pMCParam, curr_blk );
#else
      LumaResidualCoding8x8_PRED( currMB, uiBitSize - 1, uiPositionInPic, & ( currMB->cbp ), & ( currMB->cbp_blk ), block8x8, fw_mode[block8x8], bw_mode[block8x8], refframe, bw_ref, curr_blk );
#endif
        ...
        case PNXN:   //NxN
          {
            for ( j = 0; j < ( 1 << ( uiBitSize - 1 ) ); j++ )
            {
              ...
              {
                curr_blk[j][i] = curr_blk_PRED[j + ( block8x8 / 2 ) * ( 1 << ( uiBitSize - 1 ) )][i + ( block8x8 % 2 ) * ( 1 << ( uiBitSize - 1 ) )];
              }
            }
            sum_cnt_nonz += LumaResidualCoding8x8_TRANS ( currMB, uiBitSize - 1, uiPositionInPic, & ( currMB->cbp ), & ( currMB->cbp_blk ), block8x8, fw_mode[block8x8], bw_mode[block8x8], refframe, bw_ref, curr_blk );
            break;
          }
        case P2NXN:   //2NxN
        case PHOR_UP:   //2N_HU
  ...
  if ( currMB->trans_size == 0 )
  {
    ...
    sum_cnt_nonz += LumaResidualCoding8x8_TRANS( currMB, uiBitSize, uiPositionInPic, & ( currMB->cbp ), & ( currMB->cbp_blk ), 0, fw_mode_2N, bw_mode_2N, 0, 0, curr_blk_PRED );
    if ( currMB->cbp != 0 )
    {
      currMB->cbp = 15;
    }
  ...
  if ( sum_cnt_nonz <= 5 )
  {
    ...
    for ( i = 0; i < ( 1 << uiBitSize ); i++ )
    {
      for ( j = 0; j < ( 1 << uiBitSize ); j++ )
      {
        imgY[pixInPic_y + j][pixInPic_x + i] = img->predBlock[j][i];
      }
    }
#if DMH_FAST_ENC
...
void OneComponentChromaPrediction4x4 ( int uiPositionInPic, int b8, int* mpred, int pix_c_x, int pix_c_y, int step_h, int step_v, int* mv, int ref, int blocktype, int uv, int directforword )
{
  int     i, j, ii, jj;
  int     incr;
  int*    mvb;
      ...
      for (k = 0; k < 2; k++)
      {
        ...
        if (posy == 0) {
          ...
          mpred_tmp[k] = (tmp[0] * IF_CHROMA_COEF[posy][0] +
            tmp[1] * IF_CHROMA_COEF[posy][1] +
            tmp[2] * IF_CHROMA_COEF[posy][2] +
            tmp[3] * IF_CHROMA_COEF[posy][3] + 32) / 64;
#endif
        }
      }
#if IF_CHROMA_4TAP_FIX
...
#else
      *mpred++ = (mpred_tmp[0] + mpred_tmp[1] + 64) / 128;
#endif
    }
  }
}
...
*************************************************************************
*/
void OneComponentChromaPrediction4x4_dir( int uiPositionInPic, int b8, int* mpred, int pix_c_x, int pix_c_y, int step_h, int step_v, int* mv, int ref, int blocktype, int uv, int refframe )
{
  int     i, j, ii, jj;
  int     incr;
  int*    mvb;
  ...
  for ( j = pix_c_y; j < je; j++ )
  {
    for ( i = pix_c_x; i < ie; i++ )
    {
...

      {
        ...
        TRp = refframe == 0 ? delta_P : (2 * (img->imgtr_next_P - img->imgtr_fwRefDistance[1]) + 512) % 512;
        delta_PB = 2 * (picture_distance - img->imgtr_fwRefDistance[0]);
        TRp = (TRp + 512) % 512;
        delta_PB = (delta_PB + 512) % 512;
        DistanceIndexFw = delta_PB;
...
void ChromaPrediction4x4 ( codingUnit* currMB, int uiBitSize, int uiPositionInPic, int b8, int uv, int block_x, int block_y, int step_h, int step_v, int fw_mode, int bw_mode, int fw_ref_frame, int bw_ref_frame, MCParam *pMCParam )
{
  ...
  int  picC_pix_y = input->chroma_format == 1 ? ( ( uiPositionInPic / img->PicWidthInMbs ) * MIN_CU_SIZE / 2 ) : ( ( uiPositionInPic / img->PicWidthInMbs ) * MIN_CU_SIZE );
  ...
  {    
		...
		OneComponentChromaPrediction4x4 ( uiPositionInPic, b8, fw_pred, pic_pix_x, pic_pix_y, step_h, step_v, mv , ( directforward ? 0 : fw_ref_frame ), fw_mode, uv, ( directforward ? 1 : 0 ) );

#if ChromaWSM
    if ( img->type == F_IMG && num_skipmode && fw_mode == 0 )
    {
      int mv_wgt[3];
      mv_wgt[0] = ( delta_P[num_skipmode] * fmv_array[ b8 / 2 ][ b8 % 2 ][ref][fw_mode][0] * ( MULTI / delta_P[0] ) + HALF_MULTI ) >> OFFSET;
      mv_wgt[1] = ( delta_P[num_skipmode] * fmv_array[ b8 / 2 ][ b8 % 2 ][ref][fw_mode][1] * ( MULTI / delta_P[0] ) + HALF_MULTI ) >> OFFSET;
#if HALF_PIXEL_COMPENSATION_M1
      //assert(is_field_sequence == img->is_field_sequence);
      if (img->is_field_sequence)
      {
        ...
        getDeltas( &delta, &delta2, oriPOC, oriRefPOC, scaledPOC, scaledRefPOC);
#if ChromaWSMFix
        mv_wgt[1] =( ( delta_P[num_skipmode] * (fmv_array[ b8 / 2 ][ b8 % 2 ][ref][fw_mode][1]+delta) * ( 16384 / delta_P[0] ) + 8192 ) >> 14 ) - delta2;
#else
        mv_wgt[1] =( ( delta_P[num_skipmode] * (fmv_array[ b8 / 2 ][ b8 % 2 ][ref][fw_mode][1]) * ( 16384 / delta_P[0] ) + 8192 ) >> 14 );
#endif
      }
#endif

      mv_wgt[2] = 0;
      OneComponentChromaPrediction4x4 ( uiPositionInPic, b8, fw_pred_wgt, pic_pix_x, pic_pix_y, step_h, step_v, mv_wgt , num_skipmode, fw_mode, uv, ( directforward ? 1 : 0 ) );
      ...
      fpred = fw_pred;
		...
		if(img->type == F_IMG && input->dhp_enabled && dual )
		{
			...
			OneComponentChromaPrediction4x4 ( uiPositionInPic, b8, fw_pred_wgt, pic_pix_x, pic_pix_y, step_h, step_v, mv , ( directforward ? 0 : bw_ref_frame ), bw_mode, uv, ( directforward ? 1 : 0 ) );
    ...
    if(  img->type == F_IMG && (pMCParam->dir == BID_P_FST || pMCParam->dir == BID_P_SND))
    {
      ...
      OneComponentChromaPrediction4x4 ( uiPositionInPic, b8, fw_pred_wgt, pic_pix_x, pic_pix_y, step_h, step_v, mv , ( directforward ? 0 : bw_ref_frame ), fw_mode, uv, ( directforward ? 1 : 0 ) );
...
void ChromaResidualCoding ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int* cr_cbp, MCParam *pMCparam )
{
  ...
  int pix_c_y = input->chroma_format == 1 ? ( ( uiPositionInPic / img->PicWidthInMbs ) * MIN_CU_SIZE / 2 ) : ( ( uiPositionInPic / img->PicWidthInMbs ) * MIN_CU_SIZE );
    ...
    for ( *cr_cbp = 0, uv = 0; uv < 2; uv++ )
    {
      ...
      for ( j0 = 0; j0 < 2; j0 ++ )
      {
        for ( i0 = 0; i0 < 2; i0 ++, block8++ )
        {
          ...
          ChromaPrediction4x4 ( currMB, uiBitSize - 2, uiPositionInPic, block8, uv, block_x, block_y, step_h, step_v, fw_mode, bw_mode, refframe, bw_ref, pMCparam); //qyu 0824
          if ( img->NoResidueDirect )
          {
            for ( j = block_y; j < block_y + step_v; j++ )
            {
              ...
              {
                imgUV[uv][pix_c_y + j][pix_c_x + i] = img->predBlock[j][i];
              }
            }
          }
          else
          {
            for ( j = block_y; j < block_y + step_v; j++ )
            {
              for ( i = block_x; i < block_x + step_h; i++ )
              {
                ...
                tmp_block[uv][j][i] = imgUV_org[uv][pix_c_y + j][pix_c_x + i] - img->predBlock[j][i];
                img->resiY[j][i] = imgUV_org[uv][pix_c_y + j][pix_c_x + i] - img->predBlock[j][i];
              }
            }
          }
        }
      ...
      if ( !img->NoResidueDirect )
      {
        ...
        g_intra_mode   = IntraPrediction;
#if EXTEND_BD
        transform_B8 ( tmp_block_88, ( uiBitSize - 1 ), currMB,1, input->b_secT_enabled, input->sample_bit_depth );
#else
        transform_B8 ( tmp_block_88, ( uiBitSize - 1 ), currMB,1, input->b_secT_enabled );
#endif
...
#else
          CurrentChromaQP = QP_SCALE_CR[ Clip3(0, 63, (CurrentQP - (8 * (input->sample_bit_depth - 8)) - MIN_QP))] + (8 * (input->sample_bit_depth - 8));
#endif
        }
        else
        {
...
#endif
#endif
        quantization( CurrentChromaQP, IntraPrediction ? 4 : 0, 4 + uv, tmp_block_88, ( uiBitSize - 1 ), uiPositionInPic, currMB, 1, DC_PRED );
        inverse_quantization( CurrentChromaQP, IntraPrediction ? 4 : 0, 4 + uv, tmp_block_88, 0, cr_cbp, ( uiBitSize - 1 ), uiPositionInPic, currMB,1 );
...
*************************************************************************
*/
void IntraChromaPrediction8x8 ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int *mb_up, int *mb_left, int*mb_up_left )
{
...
#define EP ( edgepixels + ( ( 1 << uiBitSize ) * 2 ) )
  ...
  int     img_cy            = input->chroma_format == 1 ? ( ( ( uiPositionInPic / PicWidthInMB ) * MIN_CU_SIZE ) >> 1 ) : ( ( uiPositionInPic / PicWidthInMB ) * MIN_CU_SIZE ); //img->pix_c_y;
  ...
  for ( uv = 0; uv < 2; uv++ )
  {
    ...
    if ( mb_available_up )
    {
      for ( x = 0; x < bs_x; x++ )
      {
        EP[x + 1] = imgUV[uv][img_cy - 1][img_cx + x];
      }
      ...
      if ( mb_available_up_right )
      {
        for ( x = 0; x < bs_x; x++ )
        {
          ...
          {
            EP[1 + x + bs_x] = imgUV[uv][img_cy - 1][img->width_cr - 1];
          }
          else
          {
            EP[1 + x + bs_x] = imgUV[uv][img_cy - 1][img_cx + bs_x + x];
          }
        }
      }
      else
      ...
      {
        EP[0] = imgUV[uv][img_cy - 1][img_cx-1];
      }
      else
      {
        EP[0] = imgUV[uv][img_cy - 1][img_cx];
      }
#endif
    }
    ...
    if ( mb_available_left )
    {
      for ( y = 0; y < bs_y; y++ )
      {
        EP[-1 - y] = imgUV[uv][img_cy + y][img_cx - 1];
      }
      ...
      if ( mb_available_left_down )
      {
        for ( y = 0; y < bs_y; y++ )
        {
          ...
          {
            EP[-1 - y - bs_y] = imgUV[uv][img->height_cr - 1][img_cx - 1];
          }
          else
          {
            EP[-1 - y - bs_y]  = imgUV[uv][img_cy + bs_y + y][img_cx - 1];
          }
        }
      }
      else
...
#if EXTEND_BD
     predIntraChromaAdi( EP, piPredBuf, i, uiBitSize,mb_available_up, mb_available_left, predLmode, input->sample_bit_depth);
#else
...
int MBType2Value_NoCBP ( codingUnit* currMB )
{
  if ( img->type == B_IMG || img->type == F_IMG  || img->type == P_IMG)
  {
    ...
    else if ( currMB->cuType == PNX2N || currMB->cuType == PVER_LEFT || currMB->cuType == PVER_RIGHT )  //8x16, 4x16, 12x16
    {
      return 3;
    }
...
int writeMBHeader ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int rdopt )
{
	...
	if ( img->type != INTRA_IMG ) //qyu 0822 delete  img->skip_mode_flag == 1
	{
		...
		if ( img->type == B_IMG )
		{
			if ( AEC_writting )
			{
				fprintf ( p_trace,  "B_MB mode(%2d,%2d) = %3d\n", img->mb_x, img->mb_y, currMB->cuType );
			}
		}
		else
		{
			if ( AEC_writting )
			{
				fprintf ( p_trace,   "MB mode(%2d,%2d) = %3d\n", img->mb_x, img->mb_y, currMB->cuType );
			}
		}
		...
		if ( img->type == B_IMG && ( currMB->cuType >= P2NX2N && currMB->cuType <= PVER_RIGHT ) )
		{
			...
			if ( img->type == B_IMG )
			{
				if ( AEC_writting && currMB->cuType >= P2NXN && currMB->cuType <= PVER_RIGHT )
				{
					fprintf ( p_trace,  "B_Pred_Dir0 (%2d,%2d) = (%2d)\n", img->mb_x, img->mb_y, currMB->b8pdir[0] );
					fprintf ( p_trace,  "B_Pred_Dir1 (%2d,%2d) = (%2d)\n", img->mb_x, img->mb_y, currMB->b8pdir[3] );
				}
				else if ( AEC_writting && currMB->cuType == P2NX2N )
				{
					fprintf ( p_trace,  "B_Pred_Dir (%2d,%2d) = (%2d)\n", img->mb_x, img->mb_y, currMB->b8pdir[0] );
				}
			}
			fflush( p_trace );
#endif
...
#if DHP_OPT
#if SIMP_MV
		else if ( img->type == F_IMG && input->dhp_enabled && ( img->nb_references > 1 ) &&
			( (currMB->cuType >= P2NX2N && currMB->cuType <= PVER_RIGHT && uiBitSize >  B8X8_IN_BIT) ||
			(currMB->cuType == P2NX2N                                 && uiBitSize == B8X8_IN_BIT)) )
#else
		else if ( img->type == F_IMG && input->dhp_enabled  && ( currMB->cuType >= P2NX2N && currMB->cuType <= PVER_RIGHT ) )
#endif
#else
#if SIMP_MV
		else if ( img->type == F_IMG && input->dhp_enabled  &&
			      ( (currMB->cuType >= P2NX2N && currMB->cuType <= PVER_RIGHT && uiBitSize >  B8X8_IN_BIT) ||
				    (currMB->cuType == P2NX2N                                 && uiBitSize == B8X8_IN_BIT)) )
#else
		else if ( img->type == F_IMG && input->dhp_enabled && ( img->nb_references > 1 ) && ( currMB->cuType >= P2NX2N && currMB->cuType <= PVER_RIGHT ) )
#endif
#endif
		{
			dataPart = & ( currSlice->partArr[0] );
			...
			if ( AEC_writting && currMB->cuType >= P2NXN && currMB->cuType <= PVER_RIGHT )
			{
				fprintf ( p_trace,  "P_Pred_Dir0 (%2d,%2d) = (%2d)\n", img->mb_x, img->mb_y, currMB->b8pdir[0] );
				fprintf ( p_trace,  "P_Pred_Dir1 (%2d,%2d) = (%2d)\n", img->mb_x, img->mb_y, currMB->b8pdir[3] );
			}
			else if ( AEC_writting && currMB->cuType == P2NX2N )
			{
				fprintf ( p_trace,  "P_Pred_Dir (%2d,%2d) = (%2d)\n", img->mb_x, img->mb_y, currMB->b8pdir[0] );
			}
			fflush( p_trace );
#endif
...
#if WEIGHTED_SKIP
		if ( IS_P_SKIP ( currMB ) && img->type== F_IMG && input->wsm_enabled && img->nb_references > 1 )
		{
#if WPM_ACE_FIX
            currSE->value1 = currMB->weighted_skipmode;
#else
			currSE->value1 = currMB->weighted_skipmode % 2;
#endif
#if WPM_ACE_OPT
            currSE->type   = SE_WPM1;
#else
...
#if !WPM_ACE_FIX
		if ( IS_P_SKIP ( currMB ) && img->type == F_IMG && input->wsm_enabled && img->nb_references > 2 )
		{
			currSE->value1 = currMB->weighted_skipmode / 2;
#if WPM_ACE_OPT
            currSE->type = SE_WPM2;
...
#else
	if(IS_P_SKIP(currMB) && (currMB->weighted_skipmode == 0))
#endif
	{
		dataPart = & ( currSlice->partArr[0] );
		...
		if ( AEC_writting )
		{
			fprintf ( p_trace, "mh_p_skip_mode = %3d \n", currMB->md_directskip_mode );
		}
#endif
	...
	if ( IS_P8x8 ( currMB ) )
	{
        ...
        currSE->value1  =  currSE->bitpattern = ( currMB->b8mode[i] == PNXN ) ? ( 1 + ( currMB->b8pdir[i] == BID ? 6 : currMB->b8pdir[i] ) ) : 0;
				currSE->type    = SE_MBTYPE;
				currSE->len     = 2;
				currSE->value2  = 0;
				...
				if ( AEC_writting )
				{
					fprintf ( p_trace, "8x8 mode/pdir(%2d) = %3d/%d\n", i, currMB->b8mode[i], currMB->b8pdir[i] );
				}
#endif
    ...
    if(currMB->ui_MbBitSize==B64X64_IN_BIT)
    {
      currSE->len = 0;
		...
		currSE->value2  = 0;
		currSE->value2  = ( currMB->cuType == I8MB ||  currMB->cuType == I16MB||currMB->cuType == InNxNMB || currMB->cuType == INxnNMB );
		...
		if ( AEC_writting )
		{
			fprintf ( p_trace, "Transform_Size = %d\n", currMB->trans_size );
		}
#endif
	...
	if ( IS_INTRA ( currMB ) )
	{
		...
		for ( i = 0; i < num_of_intra_block; i++ )
		{
			...
			if ( AEC_writting )
			{
				fprintf ( p_trace, "Intra mode     = %3d %d\n", currSE->value1, currSE->context );
			}
	...
	if ( AEC_writting )
	{
		fprintf ( p_trace, "Chroma intra pred mode %d\n", currMB->c_ipred_mode );
	}
...
*************************************************************************
*/
int writeMotionVector8x8 ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int  i0, int  j0, int  i1, int  j1, int  refframe, int  dmv_flag, int  fwd_flag, int  mv_mode )
{
  int            i, j, k, l, m;
  int            curr_mvd;
  ...
  int            step_h     = g_blk_size[mv_mode * 2][0] >> MIN_BLOCK_SIZE_IN_BIT == 0 ? 1 : g_blk_size[mv_mode * 2][0] >> MIN_BLOCK_SIZE_IN_BIT;
  int            step_v     = g_blk_size[mv_mode * 2][1] >> MIN_BLOCK_SIZE_IN_BIT == 0 ? 1 : g_blk_size[mv_mode * 2][1] >> MIN_BLOCK_SIZE_IN_BIT ;
    ...
    for ( i = i0; i < i1; i += step_h )
    {
      ...
      for ( k = 0; k < 2; k++ )
      {
        ...
        if ( AEC_writting )
        {
          if ( fwd_flag )
          {
            fprintf ( p_trace, "FMVD(%d) = %3d  (org_mv %3d pred_mv %3d) %d\n", k, curr_mvd, allFwMv[i][j][refindex][mv_mode][k], pred_mv[j][i][refindex][mv_mode][k], currSE->value2 );
          }
          else
          {
            fprintf ( p_trace, "BMVD(%d) = %3d  (org_mv %3d pred_mv %3d)\n", k, curr_mvd, allFwMv[i][j][refindex][mv_mode][k], pred_mv[j][i][refindex][mv_mode][k] );
          }
        }
      ...
      for ( k = 0; k < 2; k++ )
      {
        ...
        if ( fwd_flag )
        {
          if ( AEC_writting )
          {
            ...
            fprintf ( p_trace, "FMVD(%d) = %3d  (org_mv %3d pred_mv %3d) %d\n", k, curr_mvd, allFwMv[j][i][refindex][mv_mode][k], pred_mv[j][i][refindex][mv_mode][k], currSE->value2 );
        ...
        else
        {
          if ( AEC_writting )
          {
            ...
            fprintf ( p_trace, "BMVD(%d) = %3d  (org_mv %3d pred_mv %3d)\n", k, curr_mvd, allFwMv[j][i][refindex][mv_mode][k], pred_mv[j][i][refindex][mv_mode][k] );
...
int estLumaCoeff8x8 ( int ** Quant_Coeff, codingUnit* currMB, unsigned int uiBitSize, int intra_mode, int uiPositionInPic )
{
  ...
  {
    ACRun_RDOQ[xx] = ACLevel_RDOQ[xx] = 0;
  }

  run  = -1;
  if (input->useNSQT  && currMB->ui_MbBitSize > B8X8_IN_BIT &&(currMB->trans_size == 1 && IS_INTER(currMB) && (currMB->cuType == P2NXN || currMB->cuType == PHOR_UP || currMB->cuType == PHOR_DOWN)))
  {
    if (uiBitSize == 3)
    {
  ...
  else if (input->useNSQT  && currMB->ui_MbBitSize > B8X8_IN_BIT && (currMB->trans_size == 1 && IS_INTER(currMB) && (currMB->cuType == PNX2N || currMB->cuType == PVER_LEFT || currMB->cuType == PVER_RIGHT)))
  {
    if (uiBitSize == 3)
    {
  ...
  {
	  alllevel[k] = ACLevel_RDOQ[k];
	  allrun[k] = ACRun_RDOQ  [k];
  }
...
int estChromaCoeff8x8 ( int ** Quant_Coeff, codingUnit* currMB, unsigned int uiBitSize, int b8 , int uiPositionInPic )
{
  ...
  {
    ACLevel_RDOQ[xx] = ACRun_RDOQ[xx] = 0;
  }
  ...
  if ( ( cbp >> b8 ) & 0x01 )
  {
	...
	{
		level =  alllevel[k] = ACLevel_RDOQ[k];
		run   =  allrun[k] = ACRun_RDOQ  [k];
#if TRACE
		if ( AEC_writting )
		{
			fprintf( p_trace, "AC Chroma8x8 %2d: level =%3d run =%2d\n", k, level, run );
		}
#endif
	}
...
int estLastPosRate ( int lastPosX, int lastPosY, BiContextTypePtr pCTX, int isLastCG, int CGLastX, int CGLastY , int iCG, int ctxmode, unsigned int uiBitSize, int isChroma)
{
...
#else
	if( (CGx == 0 && CGy > 0) || (uiBitSize == 3 && ( iCG == 0 ) && ( ctxmode == 1 )))
#endif
	{
		SWAP(xx, yy);    
	}
	...
	{
#if BBRY_CU8
		offset = (uiBitSize == B4X4_IN_BIT) ? (ctxmode/2)*4 : ((CGx > 0 && CGy > 0) ? 0 : (  ( ctxmode/2 ) * 4 +  ( ( iCG == 0 ) ? 4 : 12 )) + 8);
#else
		offset = (uiBitSize >  ) ? ( 0 ) : ( (CGx > 0 && CGy > 0) ? 6 : ( ( iCG == 0 ) * ( ctxmode/2 + 1 )* 6 + 12 ) );
#endif
	}
	else
	{
...
#else
		offset = ( uiBitSize > 0 ) ? ( 0 ) :  ( ( iCG == 0 ) ? 6 : 12 ) ;
#endif
	}
	if(!isLastCG)
	{
...
#endif
{
...
#else
  int offset = ((iCG == 0 && pos > 12) ? 0 : 3) + indiv + 5;
#endif
  if (!isChroma)
  {
	  offset += 3;
  ...
  {   
	  ...
	  while ( 1 )
	  {
		  if ( symbol >= ( unsigned int ) ( 1 << exp_golomb_order ) )
		  {
...
#endif
{
  ...
  if(15-pos>0)
  {
...
#if BBRY_CU8
    offset = ((iCG == 0) ? (pos+ctxpos == 14 ? 0 : (1 + moddiv)) : (isChroma ? 2 : 3 + moddiv)) + (bitSize == B4X4_IN_BIT ? 0 : 3);
#else
	...
	if(!isChroma)
	{
		...
		offset = ((iCG == 0) ? (pos+ctxpos == 14 ? 0 : (1 + moddiv%3)) :  (4 + moddiv%3)) + (bitSize == B4X4_IN_BIT ? 0 : 4);
	}
  }
...
int writeLumaCoeff8x8_AEC ( int ** Quant_Coeff, codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPixInSMB_x, unsigned int uiPixInSMB_y, int intra_mode, int uiPositionInPic )
{
  ...
  int iVer=0, iHor=0;
  if (input->useNSQT  && currMB->ui_MbBitSize > B8X8_IN_BIT && ((currMB->trans_size == 1) && (currMB->cuType == P2NXN || currMB->cuType == PHOR_UP || currMB->cuType == PHOR_DOWN)))
  {
    iHor = 1;
  }
  else if (input->useNSQT  && currMB->ui_MbBitSize > B8X8_IN_BIT && ((currMB->trans_size == 1) && (currMB->cuType == PNX2N || currMB->cuType == PVER_LEFT || currMB->cuType == PVER_RIGHT)))
  {
    iVer = 1;
  }
  else
	  if (input->useSDIP && (currMB->trans_size == 1) && (currMB->cuType == InNxNMB))
	  {
		  iHor = 1;
	  }
  ...
  else
  {
    if (input->useNSQT && currMB->ui_MbBitSize > B8X8_IN_BIT&& ( currMB->cuType < 9 && (iVer || iHor)) )  //inter MB
    {
      ...
      if (iVer)
      {
		...
		if (input->useSDIP && (( currMB->cuType == InNxNMB||currMB->cuType == INxnNMB) && (iVer || iHor)) )  //inter MB
		{
			if (iHor)
			{
				if ( uiBitSize == B8X8_IN_BIT )
  ...
  for ( icoef = 0; icoef < ( ( 1 << uiBitSize ) * ( 1 << uiBitSize ) ); icoef++ )
  {
	...
	if( intra_mode && g_intraModeClassified[currMB->l_ipred_mode] == INTRA_PRED_HOR&&currMB->cuType != InNxNMB&&currMB->cuType != INxnNMB )
	{
  ...
  for ( k = 0; k <= ( ( 1 << uiBitSize ) * ( 1 << uiBitSize ) ) && level != 0; k++ )
  {
	  level = alllevel[k] = ACLevel[k]; // level
	  run   = allrun[k] = ACRun  [k]; // run
#if TRACE
	  if ( AEC_writting )
	  {
		  fprintf ( p_trace, "Luma8x8 sng(%2d) level =%3d run =%2d\n", k, level, run );
	  }
...
*************************************************************************
*/
int writeLumaCoeff8x8 ( int ** Quant_Coeff, codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPixInSMB_x, unsigned int uiPixInSMB_y, int block8x8, int intra8x8mode, int uiPositionInPic )
{
  int  rate = 0;

  rate  += writeLumaCoeff8x8_AEC ( Quant_Coeff, currMB, uiBitSize, uiPixInSMB_x, uiPixInSMB_y, intra8x8mode, uiPositionInPic );

  return rate;
}
int writeChromaCoeff8x8 ( int ** Quant_Coeff, codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPixInSMB_x, unsigned int uiPixInSMB_y, int block8x8, int intramode, int uiPositionInPic )
{
  int  rate = 0;

  rate += writeChromaCoeff8x8_AEC ( Quant_Coeff, currMB, uiBitSize, uiPixInSMB_x, uiPixInSMB_y, block8x8, uiPositionInPic );
...
int StoreMotionVector8x8 ( codingUnit* currMB, int i0, int j0, int i1, int j1, int refframe, int dmv_flag, int fwd_flag, int mv_mode, int dmh_mode )
{
  int            i, j, k, l, m;
  int            curr_mvd;
  int            bwflag     = ( ( refframe < 0 || ( !fwd_flag ) ) ? 1 : 0 );
...
int StoreMotionVector8x8_sym ( codingUnit* currMB, int i0, int j0, int i1, int j1, int refframe, int dmv_flag, int fwd_flag, int mv_mode, int pdir )
{
  int            i, j, k, l, m;
  int            curr_mvd;
  int            bwflag     = ( ( refframe < 0 || ( !fwd_flag ) ) ? 1 : 0 );
...
*************************************************************************
*/
int writePMVIdx8x8( int  i0,  int  j0,  int  i1, int  j1, int  refframe, int  dmv_flag, int  fwd_flag, int  mv_mode, int  pdir, codingUnit* currMB, int  uiPositionInPic )
{
	int            i, j;
	...
	if (AEC_writting )
	{
		...
		fprintf ( p_trace, "Position in Pic= %d, Size = %d,  type = %d Position in cu (%d, %d)\n", uiPositionInPic, currMB->ui_MbBitSize, currMB->cuType, i0, j0);
	}
...
*************************************************************************
*/
int writeMVD8x8 ( int  i0, int  j0, int  i1, int  j1, int  refframe, int  dmv_flag, int  fwd_flag, int  mv_mode, codingUnit* currMB, int  uiPositionInPic )
{
  int            i, j, k;
  int            curr_mvd;
  int            dmh_mode;
  ...
  for ( j = j0; j < j1; j += step_v )
  {
    ...
    {
#ifdef AVS2_S2_S
		if (  img->type == F_IMG && input->b_dmh_enabled && currMB->b8pdir[0] == FORWARD && currMB->b8pdir[1] == FORWARD && currMB->b8pdir[2] == FORWARD && currMB->b8pdir[3] == FORWARD && img->typeb != BP_IMG)
#else
		if ( img->type == F_IMG && input->b_dmh_enabled && currMB->b8pdir[0] == FORWARD && currMB->b8pdir[1] == FORWARD && currMB->b8pdir[2] == FORWARD && currMB->b8pdir[3] == FORWARD)
#endif
      {
#if SIMP_MV
        if ( i == 0 && j == 0 && (!(currMB->ui_MbBitSize == B8X8_IN_BIT && currMB->cuType >= P2NXN && currMB->cuType <= PVER_RIGHT)))
#else
        if ( i == 0 && j == 0 )
#endif
        {
...
int storeMotionInfo ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int pos, int dmh_mode )
{
  ...
  int** bw_refframe_array  = ( img->type == B_IMG ) ? img->bw_refFrArr : p_snd_refFrArr;

  int   step_h0         = ( g_blk_size[  currMB->cuType * 2][0] >>  MIN_BLOCK_SIZE_IN_BIT ) == 0 ? 1 : ( g_blk_size[  currMB->cuType * 2][0] >>  MIN_BLOCK_SIZE_IN_BIT );
  int   step_v0         = ( g_blk_size[  currMB->cuType * 2][1] >>  MIN_BLOCK_SIZE_IN_BIT ) == 0 ? 1 : ( g_blk_size[  currMB->cuType * 2][1] >>  MIN_BLOCK_SIZE_IN_BIT );
  ...
  if ( !IS_INTRA( currMB ) )
  {
    for ( j0 = pos; j0 < 2; j0 += step_v0 )
    {
      ...
      {
        k = j0 * 2 + i0;
		if ( ( currMB->b8pdir[k] == FORWARD || currMB->b8pdir[k] == SYM || currMB->b8pdir[k] == BID || currMB->b8pdir[k] == DUAL) && currMB->b8mode[k] != 0 )
        {
          ...
          {
            StoreMotionVector8x8_sym ( currMB, i0, j0, i0 + step_h0, j0 + step_v0, refframe, 0, 1, currMB->b8mode[k], currMB->b8pdir[k] );
          }
  ...
  if ( IS_INTERMV ( currMB ) && bframe )
  {
    for ( j0 = pos; j0 < 2; j0 += step_v0 )
    {
      ...
      {
        k = j0 * 2 + i0;
        if ( ( currMB->b8pdir[k] == BACKWARD || currMB->b8pdir[k] == SYM || currMB->b8pdir[k] == BID ) && currMB->b8mode[k] != 0 ) //has backward vector
        {
          ...
          {
            StoreMotionVector8x8_sym ( currMB, i0, j0, i0 + step_h0, j0 + step_v0, refframe, 0, 0, currMB->b8mode[k] , currMB->b8pdir[k] );
          }
          else
...
int writeFrameRef ( codingUnit* currMB, int  mode, int  i, int  j, int  fwd_flag, int  ref, int uiPositionInPic )
{
  ...
  if ( fwd_flag )
  {
    if ( AEC_writting )
    {
      fprintf ( p_trace, "Fwd Ref frame no %d\n", currSE->bitpattern );
    }
  }
  else
  {
    if ( AEC_writting )
    {
      fprintf ( p_trace, "Bwd Ref frame no %d\n", currSE->bitpattern );
    }
  }
...
int writeReferenceIndex ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int pos, int *rate_top, int *rate_bot )
{
  ...
  else
  {
    ...
    step_h0     = g_blk_size[currMB->cuType * 2][0] >> MIN_BLOCK_SIZE_IN_BIT == 0 ? 1 : g_blk_size[currMB->cuType * 2][0] >> MIN_BLOCK_SIZE_IN_BIT;
    step_v0     = g_blk_size[currMB->cuType * 2][1] >> MIN_BLOCK_SIZE_IN_BIT == 0 ? 1 : g_blk_size[currMB->cuType * 2][1] >> MIN_BLOCK_SIZE_IN_BIT;
  ...
  for ( j0 = pos; j0 < 2; )
  {
    for ( i0 = 0; i0 < 2; )
    {
      k = j0 * 2 + i0;
      no_bits = 0;
      if ( ( currMB->b8pdir[k] == FORWARD || currMB->b8pdir[k] == SYM || currMB->b8pdir[k] == BID || currMB->b8pdir[k] == DUAL ) && currMB->b8mode[k] != 0 && img->type != INTRA_IMG )
      {
    ...
    for ( i0 = 0; i0 < 2; )
    {
      ...
      if ( ( currMB->b8pdir[k] == FORWARD || currMB->b8pdir[k] == SYM || currMB->b8pdir[k] == BID || currMB->b8pdir[k] == DUAL ) && currMB->b8mode[k] != 0 )
      {
...
#else
        if ( ( currMB->b8pdir[k] == SYM || currMB->b8pdir[k] == BID || currMB->b8pdir[k] == DUAL ) && input->InterlaceCodingOption == FRAME_CODING )
#endif
        {
          ...
          no_bits  += writeMotionVector8x8_sym ( i0, j0, i0 + step_h0, j0 + step_v0, refframe, 0, 1, currMB->b8mode[k], currMB->b8pdir[k], currMB,  uiPositionInPic );
        }
        else

        {
          ...
          img->subblock_x = start_x; // position used for context determination
          img->subblock_y = start_y; // position used for context determination
          no_bits   = writeMVD8x8 ( i0, j0, i0 + step_h0, j0 + step_v0, refframe, 0, 1, currMB->b8mode[k], currMB, uiPositionInPic );
        }
      }
...
#endif
          {
            no_bits  = writeMotionVector8x8_sym ( i0, j0, i0 + step_h0, j0 + step_v0, refframe, 0, 0, currMB->b8mode[k], currMB->b8pdir[k], currMB,  uiPositionInPic );
          }
          else
            no_bits   = writeMVD8x8 ( i0, j0, i0 + step_h0, j0 + step_v0, refframe, 0, 0, currMB->b8mode[k], currMB, uiPositionInPic );
        }
...
*************************************************************************
*/
int writeBlockCoeff ( int ** Quant_Coeff, codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPixInSMB_x, unsigned int uiPixInSMB_y, int block8x8, int uiPositionInPic )
{
  int             rate      = 0;
  ...
  if ( block8x8 < 6 )
  {
    if ( currMB->cbp & ( 1 << block8x8 ) )
    {
      if ( block8x8 < 4 )
      {
        ...
        rate  += writeLumaCoeff8x8 ( Quant_Coeff, currMB, uiBitSize, uiPixInSMB_x, uiPixInSMB_y, block8x8, ( currMB->b8mode[block8x8] == IBLOCK ), uiPositionInPic ); //changed by lzhang
      }
      else
      {
#if INTRA_2N
        if ( currMB->trans_size == 0 )
        {
          rate += writeChromaCoeff8x8 ( Quant_Coeff, currMB, uiBitSize - 1, uiPixInSMB_x, uiPixInSMB_y, block8x8, IS_INTRA ( currMB ), uiPositionInPic ); //changed by lzhang
        }
        else
        {
          rate += writeChromaCoeff8x8 ( Quant_Coeff, currMB, uiBitSize, uiPixInSMB_x, uiPixInSMB_y, block8x8, IS_INTRA ( currMB ), uiPositionInPic ); //changed by lzhang
        }
#else
        rate += writeChromaCoeff8x8 ( Quant_Coeff, currMB, uiBitSize, uiPixInSMB_x, uiPixInSMB_y, block8x8, IS_INTRA ( currMB ), uiPositionInPic ); //changed by lzhang
#endif
...
void write_one_subMB ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int eos_bit )
{
  ...
  int HeightInLCU =  (( img->height >> input->g_uiMaxSizeInBit) + (img->height % ( 1<< input->g_uiMaxSizeInBit) ? 1 : 0));
  ...
  if (input->useDQP)
  {
	  if (currMB->cbp != 0)
	  {
		  currMB->delta_qp = currMB->qp-lastQP;
		  lastQP = currMB->qp;
	  }
	  else
	  {
  ...
  if ( input->chroma_format == 1 )
  {
      ...
      (  currMB->cbp != 0 ) )
    {
      ...
      for ( i = 0; i < 6; )
      {
        if ( i < 4 )
        {
          ...
          else if (input->useNSQT && currMB->ui_MbBitSize > B8X8_IN_BIT&& (currMB->trans_size == 1 && IS_INTER(currMB) && (currMB->cuType ==PNX2N || currMB->cuType == PVER_LEFT || currMB->cuType == PVER_RIGHT)))
          {
            ...
            if (input->useSDIP && (currMB->trans_size == 1 && IS_INTRA(currMB) && (currMB->cuType ==InNxNMB || currMB->cuType == INxnNMB )))
            {
		...
		writeBlockCoeff ( img->Coeff_all_to_write_ALF[currSMB_nr], currMB, uiBitSize - currMB->trans_size/*1*/, uiPixInSMB_x, uiPixInSMB_y, i, uiPositionInPic );
      ...
      for ( i = 0; i < 6; i++ )
      {
        if ( i < 4 )
        {
          uiPixInSMB_x = uiPixInSMB_x_luma + ( i % 2 ) * ( 1 << ( uiBitSize - 1 ) ) ;
          uiPixInSMB_y = uiPixInSMB_y_luma + ( i / 2 ) * ( 1 << ( uiBitSize - 1 ) ) ;
        }
        else if ( i == 4 )
        {
          uiPixInSMB_x = ( uiPixInSMB_x_luma >> 1 ) ;
		...
		writeBlockCoeff ( img->Coeff_all_to_write_ALF[currSMB_nr], currMB, uiBitSize - 1, uiPixInSMB_x, uiPixInSMB_y, i, uiPositionInPic  );
	  }
#endif
    }
...
void write_one_SMB ( unsigned int uiBitSize, unsigned int uiPositionInPic, int eos_bit )
{
  ...
  if ( !iBoundary_start )
  {
    ...
    {
      fprintf ( p_trace, "\n*********** Pic: %i (I/P) MB: %i Slice: %i **********\n\n", img->tr, uiPositionInPic, img->current_slice_nr );
    }
...
#else
    if ( uiBitSize > B16X16_IN_BIT && !iBoundary_end )
#endif
    {
    ...
    if ( !iBoundary_end )
    {
	  ...
	  if(currMB->delta_qp==0)
		  zeroDqpCount++;  // added by KY, calculate the number of zero DQP
#endif
    }
    else
    {
