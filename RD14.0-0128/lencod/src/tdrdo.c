...
void SearchMseMean(DL *omcplist,int FrameNumberInFile, int StepLength, int IntraPeriod, FD * p1bhFD, FD *p2bhFD)  
{
	...
	if(subframenumIndex==0 && IntraPeriod==0)
	{
		...
		p2bhFD->BlockDistortionArray=omcplist->FrameDistortionArray[keyframenum].subFrameDistortionArray[0].BlockDistortionArray;
		...
		else p1bhFD->BlockDistortionArray=p2bhFD->BlockDistortionArray;
	}
	if(subframenumIndex==0 && IntraPeriod!=0)
	{
		p1bhFD->BlockDistortionArray=omcplist->FrameDistortionArray[keyframenum-1].subFrameDistortionArray[0].BlockDistortionArray;
		if(FrameNumberInFile < (int)(omcplist->TotalFrameNumber-1)*StepLength)
			p2bhFD->BlockDistortionArray=omcplist->FrameDistortionArray[keyframenum].BlockDistortionArray;
		else p2bhFD->BlockDistortionArray=p1bhFD->BlockDistortionArray;

	}

	if(subframenumIndex!=0 && IntraPeriod==0)
	{
		p2bhFD->BlockDistortionArray=p1bhFD->BlockDistortionArray=omcplist->FrameDistortionArray[keyframenum].subFrameDistortionArray[subframenumIndex].BlockDistortionArray;
	}
	if(subframenumIndex!=0 && IntraPeriod!=0)
	{
		for(i=0;i<StepLength-1;i++)
		{
			...
			if(subframenum==FrameNumberInFile)
			{
				p2bhFD->BlockDistortionArray=omcplist->FrameDistortionArray[keyframenum].subFrameDistortionArray[(i+1)*2].BlockDistortionArray;
				p1bhFD->BlockDistortionArray=omcplist->FrameDistortionArray[keyframenum].subFrameDistortionArray[(i+1)*2-1].BlockDistortionArray;
				break;
			}
		}
	}
