...
double RDCost_for_AVSIntraBlocks ( codingUnit * currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int *nonzero, int b8, int ipmode, double lambda, double  min_rdcost, int * mostProbableMode, int isChroma )
{
  ...
  if(currMB->cuType==InNxNMB||currMB->cuType==INxnNMB)
    for ( y = 0; y < ( currMB->cuType==InNxNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); y++ )
    {
      ...
      {
        tmp_block_88_inv[y][x] = img->resiY[y][x];      //the subblock to be processed is in the top left corner of img->resiY[][].
...
#if EXTEND_BD
    transform_NSQT ( tmp_block_88_inv, uiBitSize, currMB, 0, input->b_secT_enabled, input->sample_bit_depth );
#else
    transform_NSQT ( tmp_block_88_inv, uiBitSize, currMB, 0, input->b_secT_enabled );
#endif
  else
#if EXTEND_BD
		transform_B8 ( tmp_block_88_inv, uiBitSize, currMB, 0, input->b_secT_enabled, input->sample_bit_depth) ;
#else
		transform_B8 ( tmp_block_88_inv, uiBitSize, currMB, 0, input->b_secT_enabled) ;
#endif
  g_block8x8     = b8;
...
#if MB_DQP
  quantization(currMB->qp - MIN_QP, 4, b8, tmp_block_88_inv, uiBitSize, uiPositionInPic, currMB, 0, currMB->l_ipred_mode);
  inverse_quantization(currMB->qp - MIN_QP, 4, b8, tmp_block_88_inv, 0, &tmp_cbp, uiBitSize, uiPositionInPic, currMB,0);
  inverse_transform( b8, tmp_block_88_inv, uiBitSize, uiPositionInPic, currMB, 0);
#else
  quantization(img->qp - MIN_QP, 4, b8, tmp_block_88_inv, uiBitSize, uiPositionInPic, currMB, 0, currMB->l_ipred_mode);
  inverse_quantization(img->qp - MIN_QP, 4, b8, tmp_block_88_inv, 0, &tmp_cbp, uiBitSize, uiPositionInPic, currMB,0);
  inverse_transform( b8, tmp_block_88_inv, uiBitSize, uiPositionInPic, currMB, 0);
...
#if INTRA_2MPM_FIX
  currSE->value1 = ( mostProbableMode[0] == ipmode ) ? -2 : (( mostProbableMode[1] == ipmode ) ? -1 : (ipmode < mostProbableMode[0] ? ipmode : (ipmode < mostProbableMode[1] ? ipmode - 1 : ipmode - 2)));
#else
  currSE->value1 = ( mostProbableMode[0] == ipmode ) ? -2 : (( mostProbableMode[1] == ipmode ) ? -1 : (ipmode < mostProbableMode[0] ? ipmode : ipmode - 1));
#endif
...
int Mode_Decision_for_AVS_IntraBlocks ( codingUnit * currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int b8, double lambda, int *min_cost )
{
  ...
  if ( currMB->cuType==InNxNMB ||currMB->cuType==INxnNMB)//add yuqh20130824
  {
    ...
    get_mem2Dint ( &rec8x8, currMB->cuType==InNxNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ), currMB->cuType==INxnNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); //qyu 0823
    get_mem2Dint ( &Coeff_for_intra_luma, currMB->cuType==InNxNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ), currMB->cuType==INxnNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); //qyu 0823
  }
  else
  {
  ...
  *min_cost = ( 1 << 20 );
...
#else
    mostProbableMode[1]=(mostProbableMode[0]!=DC_PRED) ? DC_PRED : BI_PRED;
#endif
  }
  ...
  for (imode = 0; imode < NUM_INTRA_PMODE; imode++)
  {
    if(currMB->cuType==InNxNMB||currMB->cuType==INxnNMB)
    {
      for ( j = 0; j < ( currMB->cuType==InNxNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); j++ )
      {
        ...
        {
          img->resiY[j][i] = imgY_org[pic_pix_y + incr_y * j + off_y][pic_pix_x + i] - img->predBlockY[imode][j][i] ;
        }
      }
...
#else
  for ( ipmode = 0; ipmode < NUM_INTRA_PMODE; ipmode++ )
  {
#endif
      if ( input->rdopt )
      {
        ...
        if(currMB->cuType==InNxNMB||currMB->cuType==INxnNMB)
          for ( j = 0; j < ( currMB->cuType==InNxNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); j++ )
          {
            for ( i = 0; i < ( currMB->cuType==INxnNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); i++ )
            {
              ...
              img->resiY[j][i] = imgY_org[pic_pix_y + incr_y * j + off_y][pic_pix_x + i] - img->predBlock[block_y + j][block_x + i] ;
            }
          }
        else
          for ( j = 0; j < ( 1 << uiBitSize ); j++ )
        ...
        if ( ( rdcost = RDCost_for_AVSIntraBlocks ( currMB, uiBitSize, uiPositionInPic, &c_nz, b8, ipmode, lambda, min_rdcost, mostProbableMode, 0 ) ) < min_rdcost )
        {
          //--- set coefficients ---
          if(currMB->cuType==InNxNMB ||currMB->cuType==INxnNMB)
            for ( y = 0; y < ( currMB->cuType==InNxNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); y++ )
          ...
          if(currMB->cuType==InNxNMB ||currMB->cuType==INxnNMB)
          {
            for ( y = 0; y < ( currMB->cuType==InNxNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); y++ )
            {
              for ( x = 0; x < ( currMB->cuType==INxnNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); x++ )
              {
                rec8x8[y][x] = imgY[pic_pix_y + incr_y * y + off_y][pic_pix_x + x];
              }
            }
  ...
  if(currMB->cuType==InNxNMB||currMB->cuType==INxnNMB)
  {
      for ( j = 0; j < ( currMB->cuType==InNxNMB ?  1 << (uiBitSize-1-MIN_BLOCK_SIZE_IN_BIT) : 1 << (uiBitSize+1-MIN_BLOCK_SIZE_IN_BIT ) ); j++ )
      {
        for ( i = 0; i < ( currMB->cuType==INxnNMB ?  1 << (uiBitSize-1-MIN_BLOCK_SIZE_IN_BIT)  :1 << (uiBitSize+1 -MIN_BLOCK_SIZE_IN_BIT) ); i++ )
        {
...
#if INTRA_2MPM_FIX
  currMB->intra_pred_modes[b8] = ( mostProbableMode[0] == best_ipmode ) ? -2 : (( mostProbableMode[1] == best_ipmode ) ? -1 : (best_ipmode < mostProbableMode[0] ? best_ipmode : (best_ipmode < mostProbableMode[1] ? best_ipmode - 1 : best_ipmode - 2)));
#else
  currMB->intra_pred_modes[b8] = ( mostProbableMode[0] == best_ipmode ) ? -2 : (( mostProbableMode[1] == best_ipmode ) ? -1 : (best_ipmode < mostProbableMode[0] ? best_ipmode : best_ipmode - 1));
#endif
  ...
  if ( input->rdopt )
  {
    ...
    if(currMB->cuType==InNxNMB||currMB->cuType==INxnNMB)
    {
      for ( y = 0; y < ( currMB->cuType==InNxNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); y++ )
      {
        for ( x = 0; x < ( currMB->cuType==INxnNMB ? ( 1 << (uiBitSize-1) ) :( 1 << (uiBitSize+1) ) ); x++ )
        {
          imgY[pic_pix_y + incr_y * y + off_y][pic_pix_x + x] = rec8x8[y][x];
        }
      }
    }
    else
...
void xUpdateCandList( int uiMode, double uiCost, int uiFullCandNum, int * CandModeList, double * CandCostList )
{
  ...
  while ( shift < uiFullCandNum && uiCost < CandCostList[ uiFullCandNum - 1 - shift ] ) shift++;
...
*************************************************************************
*/
int Mode_Decision_for_NxNIntraBlocks ( codingUnit * currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int b8, double lambda, int *cost )
{
  int  nonzero = 0;
  int  cost8x8;
...
#if INTRA_2N
int Mode_Decision_for_IntracodingUnit ( codingUnit *currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, double lambda,  int* cost )
{
...
*************************************************************************
*/
double RDCost_for_8x8blocks ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int* cnt_nonz, int* cbp_blk, double lambda, int block, int mode, int pdir, int ref, int bwd_ref ) // <-- abp type
{
  int  i, j;
  ...
  if ( direct )
  {
	  ...
	  *cnt_nonz = LumaResidualCoding8x8 ( currMB, uiBitSize, uiPositionInPic, &cbp, cbp_blk, block, 0, 0,
		  max ( 0, frefarr[block_8y + ( pay >> 3 ) ][block_8x + ( pax >> 3 ) ] ), 0, &cMCParam ); /////////qyu now
#else 
	  *cnt_nonz = LumaResidualCoding8x8 ( currMB, uiBitSize, uiPositionInPic, &cbp, cbp_blk, block, 0, 0,
		  max ( 0, frefarr[block_8y + ( pay >> MIN_BLOCK_SIZE_IN_BIT ) ][block_8x + ( pax >> MIN_BLOCK_SIZE_IN_BIT) ] ), 0, &cMCParam ); /////////qyu now
#endif 
#else
    *cnt_nonz = LumaResidualCoding8x8 ( currMB, uiBitSize, uiPositionInPic, &cbp, cbp_blk, block, 0, 0,
                                        max ( 0, frefarr[block_8y + ( pay >> 3 ) ][block_8x + ( pax >> 3 ) ] ), 0 ); /////////qyu now
#endif
  }
  else
  {
...
#if WEIGHTED_SKIP
    *cnt_nonz = LumaResidualCoding8x8 ( currMB, uiBitSize, uiPositionInPic, &cbp, cbp_blk, block, fw_mode, bw_mode, ref, bwd_ref, &cMCParam );
#else
    *cnt_nonz = LumaResidualCoding8x8 ( currMB, uiBitSize, uiPositionInPic, &cbp, cbp_blk, block, fw_mode, bw_mode, ref, bwd_ref );
#endif
  }
  ...
  for ( j = 0; j < ( 1 << uiBitSize ); j++ ) /*lgp*/
  {
    ...
    {
      distortion += img->quad [imgY_original[pix_y + pay + incr_y * j + off_y][pix_x + i] - imgY[pix_y + pay + incr_y * j + off_y][pix_x + i]];
    }
  }
  ...
  if ( *cnt_nonz )
  {
    ...
    rate += writeLumaCoeff8x8 ( img->Coeff_all, currMB, uiBitSize, pax, pay, block, 0, uiPositionInPic ); //block_8x,block_8y
          ...
          for ( r = 0; r < step_v; r++ )
          {
            for ( c = 0; c < step_h; c++ )
            {
             if(pMCParam->dir == 0)
			 {
				 ...
				 p_snd_refFrArr [block_8y + row + r][block_8x + col + c] =  -1 ;
			 }
			 else if(pMCParam->dir == BID_P_FST || pMCParam->dir == BID_P_SND)
			 {
				 frefarr[block_8y + row + r][block_8x + col + c] = img->tmp_pref_fst[pMCParam->dir];
				 p_snd_refFrArr[block_8y + row + r][block_8x + col + c] = img->tmp_pref_snd[pMCParam->dir];
			 }
			 ...
			 {
				 p_snd_refFrArr [block_8y + row + r][block_8x + col + c] =  pMCParam->num_skipmode ;
			 }
			}
          }
        }
      ...
      for ( j = stage_block8x8_pos / 2/*lgp*/; j < 2; j++ )
      {
        for ( i = 0; i < 2; i++ )
        {
          ...
          for ( r = 0; r < step_v; r++ )
          {
            ...
            {
              k = 2 * j + i;
			  frefarr [block_8y + row + r][block_8x + col + c] = ( ( currMB->b8pdir[k] == FORWARD && (mode!=PNXN || best8x8mode[k]!=0)) ? best8x8ref[mode][k] : ( currMB->b8pdir[k] == DUAL && (mode!=PNXN || best8x8mode[k]!=0)) ? best8x8dualref[mode][k][0] : -1 );
			  p_snd_refFrArr[block_8y + row + r][block_8x + col + c] = ( currMB->b8pdir[k] == DUAL && (mode!=PNXN || best8x8mode[k]!=0)) ? best8x8dualref[mode][k][1] : -1;
            }
          }
        }
      }
...
void SetMotionVectorsMB ( codingUnit* currMB, int bframe, unsigned int uiBitSize, unsigned int uiPositionInPic )
{
  ...
  int ***bwMV      = img->bw_mv;
  int **refar      = refFrArr;
  int **p_snd_refar      = p_snd_refFrArr;
  ...
  for ( j = stage_block8x8_pos / 2; j < 2; j++ )
    ...
    if( img->type == F_IMG && mode8 == 0 && currMB->weighted_skipmode != 0)
    {
      for(ii = 0; ii < 2; ii ++)
        ...
        {
#if MV_SCALING_UNIFY
          allBwMv[ii][jj][0][0][0] = ( int ) (( delta_P[currMB->weighted_skipmode] * allFwMv[ii][jj][0][0][0] * (MULTI / delta_P[0]) + HALF_MULTI) >> OFFSET );
          allBwMv[ii][jj][0][0][1] = ( int ) (( delta_P[currMB->weighted_skipmode] * allFwMv[ii][jj][0][0][1] * (MULTI / delta_P[0]) + HALF_MULTI) >> OFFSET );
#else
          allBwMv[ii][jj][0][0][0] = ( int ) ( delta_P[currMB->weighted_skipmode] * allFwMv[ii][jj][0][0][0] * (16384 / delta_P[0]) + 8192 >> 14 );
          allBwMv[ii][jj][0][0][1] = ( int ) ( delta_P[currMB->weighted_skipmode] * allFwMv[ii][jj][0][0][1] * (16384 / delta_P[0]) + 8192 >> 14 );
#endif
          ...
          if (img->is_field_sequence)
          {
            ...
            allBwMv[ii][jj][0][0][1] = ( int ) ( (( delta_P[currMB->weighted_skipmode] * (allFwMv[ii][jj][0][0][1]+deltaT) * (16384 / delta_P[0]) + 8192) >> 14 ) -delta2 );
          }
#endif
#if m3469
          ...
          else if ( mode8 != 0 ) // symirect
          {
            ...
            else
            {
...
#if MV_SCALING_UNIFY
              bwMV [by + r][bx + c][0] = - ( ( allFwMv[j][i][ref][mode8][0] * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET );
              bwMV [by + r][bx + c][1] = - ( ( allFwMv[j][i][ref][mode8][1] * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET );
#else
              bwMV [by + r][bx + c][0] = - ( ( allFwMv[j][i][ref][mode8][0] * DistanceIndexBw * ( 512 / DistanceIndexFw ) + 256 ) >> 9 );
              bwMV [by + r][bx + c][1] = - ( ( allFwMv[j][i][ref][mode8][1] * DistanceIndexBw * ( 512 / DistanceIndexFw ) + 256 ) >> 9 );
#endif
              ...
              if (img->is_field_sequence )
              {
                ...
                bwMV [by + r][bx + c][1] = - ( ( (allFwMv[j][i][ref][mode8][1]+delta) * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET ) -delta2;
              }
#endif
#if m3469
...
void store_codingUnit_parameters ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, int mode )
{
  int    i, j;
  int    bframe   = ( img->type == B_IMG );
  int    **frefar = ( ( img->type == B_IMG ) ? img->fw_refFrArr : refFrArr );
  int    **brefar = img->bw_refFrArr;
  int    block_8x  = ( uiPositionInPic % img->PicWidthInMbs ) << 1; //img->block8_x;
  int    block_8y  = ( uiPositionInPic / img->PicWidthInMbs ) << 1; //img->block8_y;
  ...
  for ( j = 0; j < ( 1 << uiBitSize ); j++ )
  {
    for ( i = 0; i < ( 1 << uiBitSize ); i++ )
    {
      rec_mbY[j][i] = imgY[pix_y + j][pix_x + i];
    }
  }
  ...
  if ( input->chroma_format == 1 )
  {
    for ( j = 0; j < ( 1 << ( uiBitSize - 1 ) ); j++ )
    {
      ...
      {
        rec_mbU[j][i] = imgUV[0][pix_c_y + j][pix_c_x + i];
        rec_mbV[j][i] = imgUV[1][pix_c_y + j][pix_c_x + i];
      }
    }
  }
    ...
    if(best_mode == InNxNMB)
    {
      for ( j = stage_block8x8_pos / 2; j < 2; j++ )
      {
        for ( i = 0; i < 2; i++ )
        {
          ...
          best_ipredmode_tmp[j][i] = img->ipredmode[  1 + block_8y + ((j<<1)+i) * ( 1 << ( uiBitSize - MIN_CU_SIZE_IN_BIT-1 ) )  ][ 1 + block_8x ];  //qyu 0829
    ...
    else if(best_mode == INxnNMB)
    {
      for ( j = stage_block8x8_pos / 2; j < 2; j++ )
      {
        for ( i = 0; i < 2; i++ )
        {
          ...
          best_ipredmode_tmp[j][i] =  img->ipredmode[  1 + block_8y   ][ 1 + block_8x+ ((j<<1)+i) * ( 1 << ( uiBitSize - MIN_CU_SIZE_IN_BIT-1 ) ) ];  //qyu 0829
      ...
      for ( j = stage_block8x8_pos / 2; j < 2; j++ )
      {
        for ( i = 0; i < 2; i++ )
        {
          best_ipredmode_tmp[j][i] = img->ipredmode[  1 + block_8y + j * ( 1 << ( uiBitSize - MIN_CU_SIZE_IN_BIT ) ) ][ 1 + block_8x + i * ( 1 << ( uiBitSize - MIN_CU_SIZE_IN_BIT ) ) ];  //qyu 0829
        }
      }
  }
  ...
  for ( j = stage_block8x8_pos / 2; j < 2; j++ )
  {
    for ( i = 0; i < 2; i++ )
    {
	  ...
	  if( img->type == F_IMG )
		  p_snd_frefframe[j][i] = p_snd_refFrArr[block_8y + start_y  ][block_8x + start_x ];
#if DHP_OPT
...
#if WEIGHTED_SKIP
int RDCost_for_codingUnits ( codingUnit * currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, double   lambda, int mode, double*  min_rdcost, MCParam *pMCParam ) // <-> minimum rate-distortion cost
#else
int RDCost_for_codingUnits ( codingUnit * currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, double   lambda, int mode, double*  min_rdcost ) // <-> minimum rate-distortion cost
#endif
{
  int        i, j; //, k, ****ip4;
  double      rdcost;
  ...
  SetModesAndRefframeForBlocks ( currMB, uiBitSize, uiPositionInPic, mode, pMCParam ); //uiBitSize,uiPositionInPic//qyu 0823
  //=====  GET COEFFICIENTS, RECONSTRUCTIONS, CBP
#if INTRA_2N
  if ( mode == I8MB || mode == I16MB || mode==InNxNMB ||mode==INxnNMB)
  {
  ...
  if ( mode == I8MB )
  {
    ...
    IntraChromaPrediction8x8 ( currMB, uiBitSize - 1, uiPositionInPic, &mb_available_up, &mb_available_left, &mb_available_up_left );
...
#endif
#if WEIGHTED_SKIP
    LumaResidualCoding ( currMB, uiBitSize, uiPositionInPic, pMCParam ); //qyu 0823 .. quantilized coefficient: img->Coeff_all
#else
    LumaResidualCoding ( currMB, uiBitSize, uiPositionInPic ); //qyu 0823 .. quantilized coefficient: img->Coeff_all
#endif
  }
  ...
  for ( tmp_cbp_luma = currMB->cbp, currMB->c_ipred_mode = 0; currMB->c_ipred_mode < NUM_INTRA_PMODE_CHROMA; currMB->c_ipred_mode++ ) //
  {
    if ( IS_INTRA ( currMB ))
    {
    ...
    if ( currMB->c_ipred_mode == DC_PRED_C || ( IS_INTRA ( currMB ) ) )
    {
      ...
      for ( block8x8 = stage_block8x8_pos; block8x8 < 4; block8x8++ )
      {
        ...
        if(currMB->cuType == InNxNMB )
        {
          ...
          for ( j = 0; j < ( 1 << ( uiBitSize - 2 ) ); j++ )
          {
            for ( i = 0; i < ( 1 << ( (uiBitSize) ) ); i++ )
            {
              distortion_blk += img->quad [imgY_orig[pix_y + block_y + incr_y * j + off_y][pix_x + block_x + i] - imgY[pix_y + block_y + incr_y * j + off_y][pix_x + block_x + i]];
            }
          }
        }
        ...
        for ( j = 0; j < k; j++ )
        {
          for ( i = 0; i < ( 1 << ( uiBitSize - 2 ) ); i++ )
          {
            distortion_blk += img->quad [imgUV_orig[0][block_y + incr_y * j + off_y + pix_c_y][block_x + i + pix_c_x] - imgUV[0][pix_c_y + block_y + incr_y * j + off_y][pix_c_x + block_x + i]];
            distortion_blk += img->quad [imgUV_orig[1][block_y + incr_y * j + off_y + pix_c_y][block_x + i + pix_c_x] - imgUV[1][pix_c_y + block_y + incr_y * j + off_y][pix_c_x + block_x + i]];
          }
        }
	  ...
	  if ( mode || tmp_cbp )
      {
        for ( block8x8 = stage_block8x8_pos; block8x8 < ( input->chroma_format == 1 ? 6 : 8 );  )
        {
          if ( cp_table[stage_block8x8_pos / 2][block8x8] )
          {
            ...
            if (input->useNSQT && currMB->ui_MbBitSize > B8X8_IN_BIT&& (block8x8 < 4 && currMB->trans_size == 1 && (currMB->cuType == P2NXN || currMB->cuType == PHOR_UP || currMB->cuType == PHOR_DOWN )))
            {
            ...
            else if (input->useNSQT && currMB->ui_MbBitSize > B8X8_IN_BIT&& (block8x8 < 4 && currMB->trans_size == 1 && (currMB->cuType == PNX2N || currMB->cuType == PVER_LEFT || currMB->cuType == PVER_RIGHT )))
            {
              if (uiBitSize == B64X64_IN_BIT)
              {
                tmp_uiBitSize--;
            ...
            if (input->useSDIP && (block8x8 < 4 && currMB->trans_size == 1 && (currMB->cuType == InNxNMB )))
            {
              block_y = block8x8 * ( 1 << ( tmp_uiBitSize - 1 ) );
              block_x = 0;
            }
	  ...
	  if ( mode || tmp_cbp )
      {
        for ( block8x8 = stage_block8x8_pos; block8x8 < ( input->chroma_format == 1 ? 6 : 8 ); block8x8++ )
        {
          if ( cp_table[stage_block8x8_pos / 2][block8x8] )
          {
            block_y = ( block8x8 / 2 ) * ( 1 << ( uiBitSize - 1 ) ); //qyu 0823
            block_x = ( block8x8 % 2 ) * ( 1 << ( uiBitSize - 1 ) );
            rate_tmp = writeBlockCoeff ( img->Coeff_all, currMB, uiBitSize - 1, block_x, block_y, block8x8, uiPositionInPic );
	  ...
	  if (!(IS_INTRA(currMB)))
      {
        ...
        for ( j = 0; j < ( 1 << uiBitSize ); j++ ) //qyu 0823
        {
          ...
          {
            tmp_Coeff_luma[j][i] = img->Coeff_all[j][i];
            tmp_Rec_Y[j][i] = imgY[pix_y + j][pix_x + i];
          }
        }
        ...
        else
        {
          for ( j = 0; j < ( 1 << uiBitSize ); j++ ) //qyu 0823
          {
            ...
            {
              img->Coeff_all[j][i] = tmp_Coeff_luma[j][i];
              imgY[pix_y + j][pix_x + i] = tmp_Rec_Y[j][i];
            }
          }
...
*************************************************************************
*/
void set_stored_codingUnit_parameters ( codingUnit* currMB, unsigned int uiBitSize, unsigned int uiPositionInPic, double RD_largeBlk, double RD_smallBlk )
{
  ...
  if ( RD_largeBlk < RD_smallBlk )
  {
...
#else
    currMB->weighted_skipmode = 2 * weighted_skipmode2 + weighted_skipmode1;
#endif
#endif
    tmp = Coeff_all_tmp_best;
    Coeff_all_tmp_best = img->Coeff_all;
    ...
    for ( j = 0; j < currMB_pix_height; j++ )
    {
      ...
      {
        imgY[pix_y + j][pix_x + i] = rec_mbY[j][i]; //save mode tmp best to IMG
    ...
    if ( input->chroma_format == 1 )
    {
      for ( j = 0; j < ( currMB_pix_height >> 1 ); j++ )
      {
        ...
        {
          imgUV[0][pix_c_y + j][pix_c_x + i] = rec_mbU[j][i];
          imgUV[1][pix_c_y + j][pix_c_x + i] = rec_mbV[j][i];
          img->recon_currU[pix_cy_InSMB + j][pix_cx_InSMB + i] = rec_mbU[j][i];
          img->recon_currV[pix_cy_InSMB + j][pix_cx_InSMB + i] = rec_mbV[j][i];
          img->Coeff_all_to_write[pix_cy_InSMB + SMB_pix_height + j][pix_cx_InSMB + i] = img->Coeff_all[j + currMB_pix_height][i]; //U
          img->Coeff_all_to_write[pix_cy_InSMB + SMB_pix_height + j][pix_cx_InSMB + ( SMB_pix_height >> 1 ) + i] = img->Coeff_all[j + currMB_pix_height][i + ( currMB_pix_height >> 1 ) ]; //V
          img->Coeff_all_to_write_ALF[currSMB_nr][pix_cy_InSMB + SMB_pix_height + j][pix_cx_InSMB + i] = img->Coeff_all[j + currMB_pix_height][i]; //U
          img->Coeff_all_to_write_ALF[currSMB_nr][pix_cy_InSMB + SMB_pix_height + j][pix_cx_InSMB + ( SMB_pix_height >> 1 ) + i] = img->Coeff_all[j + currMB_pix_height][i + ( currMB_pix_height >> 1 ) ]; //V
        }
      }
    }
...
#else
        tmpMB->weighted_skipmode = 2 * weighted_skipmode2 + weighted_skipmode1;
#endif
#endif
      ...
      for ( i = 0; i < 2; i++ )
      {
        ...
        for ( r = 0; r < height; r++ )
        {
...

          {
            ...
            if( img->type == F_IMG )
            {
              p_snd_refFrArr[block_8y + row + r][block_8x + col + c] = p_snd_frefframe[j][i];
    ...
    //===============   intra pred mode   ===============
#if INTRA_2N
    if ( img->mb_data[uiPositionInPic].cuType == I8MB || img->mb_data[uiPositionInPic].cuType == I16MB ||img->mb_data[uiPositionInPic].cuType == InNxNMB ||img->mb_data[uiPositionInPic].cuType == INxnNMB)
#else
    if ( img->mb_data[uiPositionInPic].cuType == I8MB )
#endif
    {
      for ( j = 0; j < ( 1 << ( uiBitSize - MIN_CU_SIZE_IN_BIT ) ); j++ ) //uiBitSize 4:1 5:2x2 6 4x4
      {
  ...
  else
  {
    ...
    for ( j = 0; j < ( 1 << uiBitSize ); j++ )
    {
      for ( i = 0; i < ( 1 << uiBitSize ); i++ )
      {
        imgY[pix_y + j][pix_x + i] = img->recon_currY[pix_y_InSMB + j][pix_x_InSMB + i];
      }
    }
    ...
    if ( input->chroma_format == 1 )
    {
      for ( j = 0; j < ( 1 << ( uiBitSize - 1 ) ); j++ )
      {
        for ( i = 0; i < ( 1 << ( uiBitSize - 1 ) ); i++ )
        {
          imgUV[0][pix_c_y + j][pix_c_x + i] = img->recon_currU[pix_cy_InSMB + j][pix_cx_InSMB + i];
          imgUV[1][pix_c_y + j][pix_c_x + i] = img->recon_currV[pix_cy_InSMB + j][pix_cx_InSMB + i];
        }
      }
    }
      ...
      for ( i = 0; i < 2; i++ )
      {
        ...
        for ( r = 0; r < height; r++ )
        {
          for ( c = 0; c < width; c++ )
          {
            ...
            if( img->type == F_IMG )
              p_snd_refFrArr[block_8y + row + r][block_8x + col + c] = p_snd_frefar_curr[block8_InSMB_y + row + r ][block8_InSMB_x + col + c ];
          }
        }
      }
    }
...
*************************************************************************
*/
void SetRefAndMotionVectors ( unsigned int uiBitSize, unsigned int uiPositionInPic, int block, int mode, int ref, int bw_ref, int pdir ) /*lgp*13*/
{
  int i, j;
  int     bframe       = ( img->type == B_IMG );
  ...
  int**   frefArr      = ( bframe ? img->fw_refFrArr : refFrArr );
  int**   brefArr      = ( bframe ? img->bw_refFrArr : p_snd_refFrArr);
  int***  bmvArr       = ( bframe ? img->bw_mv : img->p_snd_tmp_mv);
  int***** allBwMv_arr = ( pdir == BID && mode != PSKIPDIRECT ) ? img->allBidBwMv : pdir == DUAL ? img->allDualSndMv : img->allBwMv;
  int***  fmvArr  = ( bframe ? img->fw_mv    : img->tmp_mv );
  int***** allFwMv_arr = ( pdir == SYM && mode != PSKIPDIRECT ) ? img->allSymMv : ( pdir == BID && mode != 0 ) ? img->allBidFwMv : pdir == DUAL ? img->allDualFstMv :img->allFwMv;
...
#if MV_SCALING_UNIFY
		  bmvArr[block_y + j][block_x + i][0] = - ( ( allFwMv_arr[ j / v ][ i / h ][ref][mode][0] * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET );
		  bmvArr[block_y + j][block_x + i][1] = - ( ( allFwMv_arr[ j / v ][ i / h ][ref][mode][1] * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET );
#else
          bmvArr[block_y + j][block_x + i][0] = - ( ( allFwMv_arr[ j / v ][ i / h ][ref][mode][0] * DistanceIndexBw * ( 512 / DistanceIndexFw ) + 256 ) >> 9 );
          bmvArr[block_y + j][block_x + i][1] = - ( ( allFwMv_arr[ j / v ][ i / h ][ref][mode][1] * DistanceIndexBw * ( 512 / DistanceIndexFw ) + 256 ) >> 9 );
#endif
#if HALF_PIXEL_COMPENSATION_MVD
          if (img->is_field_sequence )
          {
              ...
              scaledRefPOC = scaledPOC - DistanceIndexBw;
              getDeltas( &delta, &delta2, oriPOC, oriRefPOC, scaledPOC, scaledRefPOC);
              bmvArr[block_y + j][block_x + i][1] = - ( ( (allFwMv_arr[ j / v ][ i / h ][ref][mode][1]+delta) * DistanceIndexBw * ( MULTI / DistanceIndexFw ) + HALF_MULTI ) >> OFFSET ) -delta2;
          }
#endif
#if m3469
...
#if DHP_OPT
void ME ( unsigned int uiBitSize, unsigned int uiPositionInPic, int mode, int bframe, int max_ref, int adjust_ref, double lambda_motion, int write_ref, int lambda_motion_factor, int* valid,    double lambda_mode, codingUnit* currMB , int dualpred_enabled ) //,int **ipredmodes)
#else
void ME ( unsigned int uiBitSize, unsigned int uiPositionInPic, int mode, int bframe, int max_ref, int adjust_ref, double lambda_motion, int write_ref, int lambda_motion_factor, int* valid,    double lambda_mode, codingUnit* currMB ) //,int **ipredmodes)
#endif
{
...

  {
    if ( valid[mode] )
    {
      ...
      {
        ForwardPred ( &fw_mcost, & best_fw_ref, &best_bw_ref,  max_ref,  adjust_ref, mode,  block,   lambda_motion,  write_ref, lambda_motion_factor, uiPositionInPic, uiBitSize );

#if DHP_OPT
#if SIMP_MV
		if ( img->type == F_IMG && input->dhp_enabled  && dualpred_enabled && max_ref > 1 && (!(uiBitSize==B8X8_IN_BIT && mode >= P2NXN && mode <= PVER_RIGHT)))
#else
		if ( img->type == F_IMG && input->dhp_enabled  && dualpred_enabled && max_ref > 1 )
#endif
#else
#if SIMP_MV
		if ( img->type == F_IMG && input->dhp_enabled   && max_ref > 1 && (!(uiBitSize==B8X8_IN_BIT && mode >= P2NXN && mode <= PVER_RIGHT)))
#else
		if ( img->type == F_IMG && input->dhp_enabled   && max_ref > 1 )
#endif
#endif
		{
			DualPred( &dual_mcost, & dual_best_fst_ref, &dual_best_snd_ref, max_ref,  adjust_ref, mode,  block,   lambda_motion,  write_ref, lambda_motion_factor, uiPositionInPic, uiBitSize );
		}
        if ( bframe )
        {
          ...
          if (uiBitSize==B8X8_IN_BIT && mode >= P2NXN && mode <= PVER_RIGHT)
            BackwardPred ( &best_bw_ref, &bw_mcost, mode,  block,  lambda_motion, max_ref,  adjust_ref, uiPositionInPic, uiBitSize );
          else
            BiPred ( &best_bw_ref, &bw_mcost, &sym_mcost, &sym_best_fw_ref, &sym_best_bw_ref, &bid_mcost, &bid_best_fw_ref, &bid_best_bw_ref, mode,  block,  lambda_motion, max_ref,  adjust_ref, uiPositionInPic, uiBitSize );
#else
          BiPred ( &best_bw_ref, &bw_mcost, &sym_mcost, &sym_best_fw_ref, &sym_best_bw_ref, &bid_mcost, &bid_best_fw_ref, &bid_best_bw_ref, mode,  block,  lambda_motion, max_ref,  adjust_ref, uiPositionInPic, uiBitSize );
#endif
        ...
        else if ( mode == P2NX2N )
        {
          ...
          best8x8syoneForthRefY [mode][0][0] = best8x8syoneForthRefY [mode][1][0] = best8x8syoneForthRefY [mode][2][0] = best8x8syoneForthRefY [mode][3][0] = sym_best_fw_ref;
          best8x8syoneForthRefY [mode][0][1] = best8x8syoneForthRefY [mode][1][1] = best8x8syoneForthRefY [mode][2][1] = best8x8syoneForthRefY [mode][3][1] = sym_best_bw_ref;

          best8x8bidref  [mode][0][0] = best8x8bidref   [mode][1][0] = best8x8bidref  [mode][2][0] = best8x8bidref   [mode][3][0] = bid_best_fw_ref;
          best8x8bidref  [mode][0][1] = best8x8bidref   [mode][1][1] = best8x8bidref  [mode][2][1] = best8x8bidref   [mode][3][1] = bid_best_bw_ref;
          best8x8dualref  [mode][0][0] = best8x8dualref   [mode][1][0] = best8x8dualref  [mode][2][0] = best8x8dualref   [mode][3][0] = dual_best_fst_ref;
          best8x8dualref  [mode][0][1] = best8x8dualref   [mode][1][1] = best8x8dualref  [mode][2][1] = best8x8dualref   [mode][3][1] = dual_best_snd_ref;
        }

        //--- set reference frames and motion vectors ---
        if ( mode > P2NX2N && block == 0 )
        {
			SetRefAndMotionVectors ( uiBitSize, uiPositionInPic, block, mode, best_pdir == SYM ? sym_best_fw_ref : best_pdir == BID ?  bid_best_fw_ref : best_pdir == DUAL ? dual_best_fst_ref : best_fw_ref,
                                   best_pdir == SYM ? sym_best_bw_ref : best_pdir == BID ? bid_best_bw_ref : best_pdir == DUAL ? dual_best_snd_ref : best_bw_ref, best_pdir );
        }
      }
    } // if (valid[mode])
...

  {
    ...
    for ( block = stage_block8x8_pos; block < 4; block++ )
    {
      ...
      for ( min_rdcost = 1e30,  index = ( bframe ? 0 : 1 ); index < 2; index++ ) // mv_range, 20071009
      {
        if ( valid[mode = b8_mode_table[index]] )
        {
          ...
          if ( bframe )
          {
            ...
            rdcost = RDCost_for_8x8blocks ( currMB, uiBitSize - 1, uiPositionInPic, &cnt_nonz, &curr_cbp_blk, lambda_mode, block, mode, best_pdir,
                                            best_pdir == SYM ? sym_best_fw_ref : best_pdir == BID ? bid_best_fw_ref : best_fw_ref, best_pdir == SYM ? sym_best_bw_ref : best_pdir == BID ? bid_best_bw_ref : best_bw_ref ); /*lgp*13*/
      ...
      if ( block < 3 )
      {
        ...
        SetRefAndMotionVectors ( uiBitSize, uiPositionInPic, block, mode, best8x8pdir[PNXN][block] == SYM ? best8x8syoneForthRefY[PNXN][block][0] :
			best8x8pdir[PNXN][block] == BID ? best8x8bidref[PNXN][block][0] : best8x8pdir[PNXN][block] == DUAL ? best8x8dualref[PNXN][block][0] : best8x8ref[PNXN][block], best8x8pdir[PNXN][block] == SYM ?
                                 best8x8syoneForthRefY[PNXN][block][1] :  best8x8pdir[PNXN][block] == BID ? best8x8bidref[PNXN][block][1] : best8x8pdir[PNXN][block] == DUAL ? best8x8dualref[PNXN][block][1] : best8x8bwref[PNXN][block], best8x8pdir[PNXN][block] );
      } // if (block<3)
...
*************************************************************************
*/
double encode_SubMB ( unsigned int uiBitSize, unsigned int uiPositionInPic, double minRdCostD, double split_cost_down, double split_cost_up, double splitRdCost )
{
  int         valid[MAXMODE];
...
#else
  if(img->type == P_IMG && img->typeb == BP_IMG && input->bg_enable && input->profile_id == 0x50)	//only one reference frame(G\GB) for S frame
#endif
  {
	  adjust_ref = 0;
	  max_ref = 1;
  ...
  if((input->useSDIP&&uiBitSize==5&&(img->pix_x+32<=img->width)&&(img->pix_y+32<=img->height))||
	  (input->useSDIP&&uiBitSize==4&&(img->pix_x+16<=img->width)&&(img->pix_y+16<=img->height)))//只对32x8和8x32和16x4和4x16
  { 
  valid[InNxNMB]  = 1;
  valid[INxnNMB]  = 1;
  }
...
#else
  valid[PHOR_UP]      = ( !intra && input->InterSearchAMP ); // 16*4
  valid[PHOR_DOWN]      = valid[4]; //16*12
  valid[PVER_LEFT]      = valid[4]; //4*16
  valid[PVER_RIGHT]      = valid[4]; //12*16
#endif
  valid[PNXN]   = ( !intra && input->InterSearch8x8 );
#if M3198_CU8
  if ( uiBitSize > MIN_CU_SIZE_IN_BIT || uiBitSize == B8X8_IN_BIT) //qyu 0906
#else
  if ( uiBitSize > 4 ) //qyu 0906
#endif
  {
    valid[PNXN] = 0;
  }
...
#else
  if(img->type == P_IMG && img->typeb == BP_IMG && input->bg_enable && input->profile_id == 0x50)	//mode restriction for S frame
#endif
  {
	  valid[PSKIPDIRECT]		= 1;
	  valid[P2NX2N]				= 0;
  ...
  if ( input->rdopt )
  {
    ...
    global_lambda = ( lambda_mode * input->lambda_factor_rdoq ) / 100;
    if (( img->type == F_IMG )||( img->type == P_IMG ))
    {
      global_lambda = ( global_lambda * input->lambda_factor_rdoq_p ) / 100;
    }
    else if ( img->type == B_IMG )
    {
      global_lambda = ( global_lambda * input->lambda_factor_rdoq_b ) / 100;
    }
...
#if AVS2_S2_FASTMODEDECISION
  if (input->bg_input_number==1&&bg_flag&&img->type!=INTRA&&img->typeb!=BP_IMG&&img->typeb!=BACKGROUND_IMG)
  {
	  ...
	  { 
		  for ( block8x8 = 0; block8x8 < 4; block8x8++ )
		  {
			  ...
			  for ( o = 0; o < ( 1 << ( uiBitSize - 1 ) ); o++ )
			  {
				  ...
				  {
					  temp =  imgY[pix_y + block_y + o][pix_x + block_x + p] - background_frame[0][pix_y + block_y + o][pix_x + block_x + p];
					  sum_diff_8x8 += (abs(temp)>3);   
				  }
				  if (block8x8 == 0)
					  sum_diff_16x16 = sum_diff_8x8;
				  else
					  sum_diff_16x16 += sum_diff_8x8;
			  }
		  }
		  sum_diff=sum_diff_16x16;
		  sum_diff_32x32 += sum_diff_16x16;


	  }
	  if(uiBitSize==5)
	  {
		  sum_diff=sum_diff_32x32;
		  sum_diff_64x64 += sum_diff_32x32;
	  }
	  if (uiBitSize==6)
	  {
		  sum_diff=sum_diff_64x64;
	  }
  ...
  if ( input->rdopt )
  {
    ...
    for ( ctr16x16 = 0, index = 0; index < 13; index++ )
    {
...
#if DHP_ENC_OPT
		ME ( uiBitSize, uiPositionInPic, mode, bframe, max_ref, adjust_ref, lambda_motion, write_ref, lambda_motion_factor, valid, lambda_mode, currMB , 1);
#else
        ME ( uiBitSize, uiPositionInPic, mode, bframe, max_ref, adjust_ref, lambda_motion, write_ref, lambda_motion_factor, valid, lambda_mode, currMB );
#endif
      }
      ...
      if(uiBitSize==MIN_CU_SIZE_IN_BIT)
      {
        if(modeRDcost[I8MB] > modeRDcost[I16MB] * 1.06 || modeRDcost[I8MB] < modeRDcost[I16MB] * 0.7)continue;
      ...
      if(uiBitSize==MIN_CU_SIZE_IN_BIT)
      {
        if(modeRDcost[I16MB] < minRdCostD*0.94 || modeRDcost[I8MB] < minRdCostD*0.96)continue;
        if(modeRDcost[I16MB] < minRdCostD && modeRDcost[I8MB] < minRdCostD)continue;
        if( modeRDcost[I8MB] > minRdCostD*1.1 && modeRDcost[I16MB] > minRdCostD*1.1)continue;

        if(modeRDcost[I8MB] < modeRDcost[I16MB] * 0.9 || modeRDcost[I8MB] > modeRDcost[I16MB] * 1.1)continue;
        if(mode ==  INxnNMB && (modeRDcost[InNxNMB]<modeRDcost[I16MB] && modeRDcost[InNxNMB] < modeRDcost[I8MB] || modeRDcost[InNxNMB]<minRdCostD ))
          continue;
      }
      else
      {
      ...
      if ( valid[mode] )
      {
		...
		if ( !( img->type == F_IMG  && input->wsm_enabled && !mode && img->nb_references > 1 ) )
        {
          max_num_skipmode = 1;
        }
        else
        {
          max_num_skipmode = img->nb_references;
        }
        for ( i = 0; i < max_num_skipmode; i++ )
        {
          currMB->weighted_skipmode = i;
#ifdef AVS2_S2_S
		  if ( img->type == F_IMG && mode != 0 && mode != I8MB && mode != I16MB&& mode != InNxNMB  && mode != INxnNMB  && mode != IBLOCK && input->b_dmh_enabled && best8x8pdir[mode][0] == FORWARD && best8x8pdir[mode][1] == FORWARD && best8x8pdir[mode][2] == FORWARD && best8x8pdir[mode][3] == FORWARD && img->typeb != BP_IMG)
#else
		  if ( img->type == F_IMG && mode != 0 && mode != I8MB && mode != I16MB&& mode != InNxNMB  && mode != INxnNMB  && mode != IBLOCK && input->b_dmh_enabled && best8x8pdir[mode][0] == FORWARD && best8x8pdir[mode][1] == FORWARD && best8x8pdir[mode][2] == FORWARD && best8x8pdir[mode][3] == FORWARD) // need to be modified for RA
#endif
		  {
       ...
       bypass_all_dmh = 0;
          ...
          {
#if DMH_FAST_ENC
            if ( dmh_mode > 0 && bypass_all_dmh )
            {
              continue;
            }
#endif

#if DMH_ENC_COMP_REDUCT
            ...
            cMCParam.dir = 0;
            currMB->md_directskip_mode = 0;
            if ( RDCost_for_codingUnits ( currMB, uiBitSize, uiPositionInPic, lambda_mode, mode, &min_rdcost, &cMCParam ) )
            {
#if WPM_ACE_FIX
                weighted_skipmode_fix = i;
#else
              weighted_skipmode1 = i % 2 ;
              weighted_skipmode2 = i / 2;
#endif
              store_codingUnit_parameters ( currMB, uiBitSize, uiPositionInPic, mode );
...
#else
        if ( RDCost_for_codingUnits ( currMB, uiBitSize, uiPositionInPic, lambda_mode, mode, &min_rdcost ) ) //qyu 0825
        {
          store_codingUnit_parameters ( currMB, uiBitSize, uiPositionInPic, mode );
        }
        else
...
#if DHP_ENC_OPT
	  if ( img->type == F_IMG && input->dhp_enabled && ( best8x8pdir[mode][0] != FORWARD || best8x8pdir[mode][1] != FORWARD || best8x8pdir[mode][2] != FORWARD || best8x8pdir[mode][3] != FORWARD ) && ( ( mode == 1 && ctr16x16 == 0 ) || ( mode > P2NX2N && mode < I8MB ) ) && max_ref > 1 )
	  {
		  if ((uiBitSize == 5 || uiBitSize == 6) && (mode == PHOR_UP || mode == PHOR_DOWN) && !(bestMode == P2NXN) )
		  {
			  continue;
		  ...
		  ME ( uiBitSize, uiPositionInPic, mode, bframe, max_ref, adjust_ref, lambda_motion, write_ref, lambda_motion_factor, valid, lambda_mode, currMB, 0 );
		  if ( valid[mode])
		  {
...
#else
					  weighted_skipmode1 = 0;
					  weighted_skipmode2 = 0;
#endif
					  store_codingUnit_parameters ( currMB, uiBitSize, uiPositionInPic, mode );
					  secBestMode = bestMode;
					  bestMode = mode;
...
#else
	  if( (pframe || bframe || img->type == F_IMG ) && mode == 0 )
#endif
	  {
...
#if B_MODE_FIX
	  if (( bframe && valid[mode] && !IS_INTRA(currMB) && currMB->cbp ) || ( ( img->type == P_IMG || img->type == F_IMG ) && mode==0 && currMB->cbp && valid[0]))
#else
...
#endif
	  {
		  ...
		  if (valid[mode])
		  {
			  ...
			  for ( i = 0; i < max_num_skipmode; i++ )
			  {
				  ...
				  cMCParam.dir = 0;
				  currMB->md_directskip_mode = 0;
				  if ( RDCost_for_codingUnits ( currMB, uiBitSize, uiPositionInPic, lambda_mode, mode, &min_rdcost, &cMCParam ) ) // qyu 0825
#else//WEIGHTED else
		  if ( RDCost_for_codingUnits ( currMB, uiBitSize, uiPositionInPic, lambda_mode, mode, &min_rdcost ) ) //qyu 0825
#endif//WEIGHTED endif
		  {
#if WPM_ACE_FIX
              weighted_skipmode_fix = i;
#else
...
#endif
   {
    ...
    set_stored_codingUnit_parameters ( currMB, uiBitSize, uiPositionInPic, min_rdcost + splitRdCost + split_cost_up, minRdCostD + splitRdCost + split_cost_down );
  }
...
double encode_one_SMB ( unsigned int uiBitSize, unsigned int uiPositionInPic, double split_rd_cost )
{
  ...
  if ( uiBitSize > MIN_CU_SIZE_IN_BIT )
  {
    ...
    if ( !iBoundary_end )
    {
      split_cost = lambda_mode * writeSplitFlag ( 1, &tmpMB, uiBitSize );
    }
  ...
  if ( !iBoundary_end )
  {
    ...
    if ( uiBitSize > MIN_CU_SIZE_IN_BIT )
    {
      split_cost_up = lambda_mode * writeSplitFlag ( 0, &tmpMB, uiBitSize );
    }
	  min_rd_cost = encode_SubMB ( uiBitSize, uiPositionInPic, min_rd_cost, split_cost, split_cost_up, split_rd_cost ); // down layer CU cost, down layer split cost, up layer split cost, previouse split cost
  }
