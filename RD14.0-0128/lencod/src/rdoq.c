...
void rdoq_cg( int qp, int mode, int **curr_blk, unsigned int uiBitSize, double lambda, int isChroma, unsigned int uiPositionInPic, int intraPredMode, codingUnit* currMB)
{
#if TRANS_16_BITS
...
// Adaptive frequency weighting quantization
  ...
  Node nodeBuf[16+1]; // buffer of linked list nodes, so that dynamic mem alloc is not necessary. this is for speed purpose
  int index = 0;
  int lastPos = -1;
  int xx, yy;
  int isLastCG=0;
...
// Adaptive frequency weighting quantization
  ...
  else if (input->useNSQT && currMB->ui_MbBitSize > B8X8_IN_BIT&& (IS_INTER(currMB) && !(isChroma) && (currMB->trans_size == 1) && ((currMB->cuType == PNX2N) || (currMB->cuType == PVER_LEFT) || (currMB->cuType == PVER_RIGHT))))
  {
    ...
    if (input->useSDIP&& (IS_INTRA(currMB) && !(isChroma)  && (currMB->trans_size == 1)&& currMB->cuType == InNxNMB  ))   //add yuqh20130824
  {     
    if ( uiBitSize == B8X8_IN_BIT )
    {
      AVS_SCAN = AVS_SCAN4x16;
  ...
  else if (input->useSDIP&& (IS_INTRA(currMB) && !(isChroma)   && (currMB->trans_size == 1)&& currMB->cuType == INxnNMB ))  //add yuqh20130824
  {          
    if ( uiBitSize == B8X8_IN_BIT )
    {
      AVS_SCAN = AVS_SCAN16x4;
  ...
  for (coeff_ctr = coeff_num - 1; coeff_ctr >= 0; coeff_ctr--)
  {
	...
	if( !isChroma && g_intraModeClassified[intraPredMode] == INTRA_PRED_HOR &&currMB->cuType != InNxNMB&&currMB->cuType != INxnNMB)
	{
		SWAP(xx, yy);
	}
...
// Adaptive frequency weighting quantization
	...
	temp = 16384.0 / Q_TAB[qp];
...
#else
		residue_bit = (blockSize == B64X64_IN_BIT) ? 10 : 9;
#endif
		...
		if (shift_16_bit >= 1)
		{
			...
			{
#if AWQ_WEIGHTING
				levelData[coeff_ctr_in_cg].levelDouble = ( ((int)absm( curr_blk[yy][xx] )* levelscale_coef) + ( 1 << (shift_16_bit - 1) ) ) >> shift_16_bit;
#else
	...
	{
#if AWQ_WEIGHTING
		levelData[coeff_ctr_in_cg].levelDouble = ( ((int)absm( curr_blk[yy][xx] )* levelscale_coef) + ( 1 << 18 ) ) >> 19;
#else
		levelData[coeff_ctr_in_cg].levelDouble = ( absm( curr_blk[yy][xx] ) * ScaleM[yy & 3][xx & 3] + ( 1 << 18 ) ) >> 19;
#endif
	}
	else
	{
	...
	{
		rec_double = ( (((((long long int)(level*wqm_coef))>>wqm_shift)*QPI)>>4) + ( 1 << ( shift5 - 1 ) ) ) >> ( shift5 );
	}
#if QuantClip
  rec_double = Clip3(-32768, 32767, rec_double);
...
#if EXTEND_BD
	lowerInt = ( ( levelData[coeff_ctr_in_cg].levelDouble2 - rec_double )/(1<<(shift_16_bit >= 0 ? shift_16_bit : 0)) <= temp ) ? 1 : 0;
#else
	lowerInt = ( ( levelData[coeff_ctr_in_cg].levelDouble2 - rec_double )/(1<<shift_16_bit) <= temp ) ? 1 : 0;
#endif
#else
    level = ( int )( levelData[coeff_ctr_in_cg].levelDouble * Q_TAB[qp] >> 15 );
#if FREQUENCY_WEIGHTING_QUANTIZATION
	if(!WeightQuantEnable)
	{
	...
	{
		rec_double = ( (((((int)(level*wqm_coef))>>wqm_shift)*QPI)>>4) + ( 1 << ( shift - 1 ) ) ) >> ( shift );
	}
#else
    rec_double = ( level * QPI + ( 1 << ( shift - 1 ) ) ) >> ( shift );
#endif

    lowerInt = ( ( levelData[coeff_ctr_in_cg].levelDouble - rec_double ) <= temp ) ? 1 : 0;
#endif
...
// Adaptive frequency weighting quantization
		...
		{
#if AWQ_WEIGHTING
			rec_double = (double)( ( (((((long long int)levelData[coeff_ctr_in_cg].level[ii]*wqm_coef)>>wqm_shift)*QPI)>>4) + ( 1 << ( shift5 - 1 ) ) ) >> ( shift5 ) );
#else
    ...
    rec_double = Clip3(-32768, 32767, rec_double);
...
#else
		levelData[coeff_ctr_in_cg].errLevel[ii] = 256 * err * err /(1<<(shift_16_bit*2));
#endif
#else
		if(WeightQuantEnable)
		{
#if AWQ_WEIGHTING
		   err =  ( double )( (((((levelData[coeff_ctr_in_cg].level[ii]*wqm_coef) >>wqm_shift) * QPI)>>4) + ( 1 << ( shift - 1 ) ) ) >> shift ) - ( double )( levelData[coeff_ctr_in_cg].levelDouble );
#else
		   err =  ( double )( (levelData[coeff_ctr_in_cg].level[ii] * QPI + ( 1 << ( shift - 1 ) ) ) >> shift ) - ( double )( levelData[coeff_ctr_in_cg].levelDouble );
#endif
		}
		else
		{
		...
		levelData[coeff_ctr_in_cg].errLevel[ii] = err * err * 256;
#endif
        ...
        while (node != NULL)
        {
          ...
          else if (node->attrib == LAST_RUN) // this is not the last CG
          {
            ...
            if(node->run!=16)
            {
              ...
              costStat[iCG].lastRunCost = lambda * estLastPosRate ( xx, yy, pCTXLastPosInCG, 0, CGx, CGy, iCG, ctxmode, uiBitSize, isChroma);
            }
            else
            {
          ...
          else  // a (level, run) pair
          {
            ...
            for (levelNo = 1; levelNo < node->levelData->noLevels; levelNo++)
            {
...
#else
              rateLevel = estLevelRate ( absLevel, pCTX, rank, pairsInCG, iCG, 15 - node->pos, isChroma );
#endif
...
#else
              rateRunCurr = estRunRate ( node->run, pCTX, 15 - node->pos, iCG, node->pos, isChroma, ctxmode );
#endif
              ...
              else // RUN_LEVEL_PAIR
              {
                ...
                pCTX = Primary[ min(absSum5/2, 2) ];
#if BBRY_CU8
                rateRunPrev = estRunRate ( node->prev->run, pCTX, 15 - node->prev->pos, iCG, node->prev->pos, isChroma, ctxmode, uiBitSize ); 
#else
            ...
            else 
            {
              if(node->prev->attrib == RUN_LEVEL_PAIR)
              {
                ...
                pCTX = Primary[ min(absSum5/2, 2) ]; 
#if BBRY_CU8
                rateRunMerged = estRunRate ( node->prev->run + 1 + node->run, pCTX, 15 - node->prev->pos, iCG, node->prev->pos, isChroma, ctxmode, uiBitSize );
#else
                rateRunMerged = estRunRate ( node->prev->run + 1 + node->run, pCTX, 15 - node->prev->pos, iCG, node->prev->pos, isChroma, ctxmode );
#endif
...
#if BBRY_CU8
	prevLastCGCost = lambda * estCGLastRate (CG_num-1, pCTXLastCG, uiBitSize - 3, &CGx, &CGy, isChroma, ctxmode, currMB, g_intraModeClassified[intraPredMode] );
#else
...
#if BBRY_CU8
	  currLastCGCost = lambda * estCGLastRate (iCG, pCTXLastCG, uiBitSize - 3, &CGx, &CGy, isChroma, ctxmode, currMB, g_intraModeClassified[intraPredMode] );
#else
...
{
  int temp[32][32], i, j;
  double lambda_rdoq = global_lambda;
  int iSizeX,iSizeY;
  ...
  else if (input->useNSQT && currMB->ui_MbBitSize > B8X8_IN_BIT&& (IS_INTER(currMB) && !(isChroma) && (currMB->trans_size == 1) && ((currMB->cuType == PNX2N) || (currMB->cuType == PVER_LEFT) || (currMB->cuType == PVER_RIGHT))))
  {
    ...
    if (input->useSDIP  && !(isChroma)&& currMB->cuType == InNxNMB && (currMB->trans_size == 1)) 
    {
      iSizeX = (uiBitSize == B32X32_IN_BIT)? (1 << (uiBitSize)): (1 << (uiBitSize + 1)); 
      iSizeY = (uiBitSize == B32X32_IN_BIT)? (1 << (uiBitSize -2 )): (1 << (uiBitSize - 1));
    }
...
#else
   rdoq_cg(qp, mode, curr_blk, uiBitSize, lambda_rdoq, isChroma, uiPositionInPic);
#endif
...
int estimate_bits( int **Quant_Coeff, unsigned int uiBitSize, int uiPositionInPic )
{
  ...
  if ( g_block8x8 > 3 )
  {
    return estChromaCoeff8x8( Quant_Coeff, &tempMB, uiBitSize, g_block8x8, uiPositionInPic );
  }
  else
  {
    return estLumaCoeff8x8( Quant_Coeff, &tempMB, uiBitSize, g_intra_mode, uiPositionInPic );
  }
}
