...
void writeCuTypeInfo ( codingUnit* currMB, SyntaxElement *se, EncodingEnvironmentPtr eep_dp, int uiPosition )
{
...
#if !NEW_CUTYPE
  const int MapPCUType[11] = {0, 2, 3, 4, 3, 3, 4, 4, 5, 1, 1};  //  0, 1, 2, 3, 4, 5, 6, 7, P8x8, I16MB, I8MB
#else 
  const int MapPCUType[11] = {0, 2, 3, 5, 3, 3, 5, 5, 4, 1, 1};  //  0, 1, 2, 3, 4, 5, 6, 7, P8x8, I16MB, I8MB
#endif 
  const int MapBCUType[11] = {5, 1, 2, 3, 2, 2, 3, 3, 4, 6, 6};
  BiContextTypePtr pCTX = ctx->cuType_contexts;
  BiContextTypePtr pAMPCTX = ctx->amp_contexts[0];
  int max_bit = 0;
  int real_bit = 0 ;
  ...
  if ( img->type != INTRA_IMG )
  {
    ...
    if ( img->type == B_IMG && currMB->cuType == 0 && currMB->cbp ==0 )
    {
      act_sym = 0;
    }
#if !NEW_CUTYPE
    ...
    {
      max_bit-- ;
      if ( img->type== B_IMG &&( IS_INTRA(currMB) ||IS_DIRECT(currMB) ))
      {
        act_sym-- ;
      }
    }
    ...
    if ( input->InterSearchAMP )
    {
      ...
      else if ( currMB->cuType >= 4 && currMB->cuType <= 7 )
      {
  ...
  if ( IS_INTRA(currMB))
  {
    biari_encode_symbol ( eep_dp, currMB->trans_size, img->currentSlice->syn_ctx->tu_contexts+1 );
  }
#endif
...
#else
		if ((( binidx == 4 && img->type == B_IMG ) || ( binidx == 5 && img->type == F_IMG ) || ( binidx == 5 && img->type == P_IMG )) && ( currMB->ui_MbBitSize != MIN_CU_SIZE_IN_BIT ))
#endif
      {
        biari_encode_symbol_final ( eep_dp, 0 );
      }
...
void writeRefFrame ( codingUnit* currMB, SyntaxElement *se, EncodingEnvironmentPtr eep_dp, int uiPosition )
{
  ...
  int   act_ctx;
  int   act_sym;
  int** refframe_array = ( ( img->type == B_IMG ) ? img->fw_refFrArr : refFrArr ); //enc_picture->ref_idx[se->value2];
  int bslice = ( img->type == B_IMG );
  int b8a, b8b;
  PixelPos block_a, block_b;
  codingUnit *leftMB, *upMB;


  int bw_flag = se->value2 & 0x01;
#if !HZC_BUGFIX
  getLuma8x8Neighbour ( img->subblock_x, img->subblock_y, -1,  0, &block_a, uiPosition, currMB->ui_MbBitSize, currMB, bw_flag );
  getLuma8x8Neighbour ( img->subblock_x, img->subblock_y,  0, -1, &block_b, uiPosition, currMB->ui_MbBitSize, currMB, bw_flag );
  ...
  else if ( IS_DIRECT ( upMB ) || ( upMB->b8mode[b8b] == 0 && bslice ) )
  {
    b = 0;
  }
  else
  {
    ...
    if ( upMB == currMB/*&&currMB->cuType==4&&currMB->ui_MbBitSize>4*/ )
    {
      //          if(block_b.pos_y - b8_y ==  (1<<(currMB->ui_MbBitSize -4 ))-1)
      {
        block_b.pos_y = b8_y;
      }
    }
  ...
  else if ( IS_DIRECT ( leftMB ) || ( leftMB->b8mode[b8a] == 0 && bslice ) )
  {
    a = 0;
  }
  else
  {
    ...
    if ( leftMB == currMB/*&&currMB->cuType==6&&currMB->ui_MbBitSize>4*/ )
    {
  ...
  else
  {
    ...
    if ( bslice == 0 )
    {
      ...
      //if( se->value1<3 )
	  if (se->value1<img->nb_references-1)
      biari_encode_symbol ( eep_dp, 1, pCTX + act_ctx );
    }
  }
...
void writeMVD_AEC ( codingUnit* currMB, SyntaxElement *se, EncodingEnvironmentPtr eep_dp, int uiPosition )
{
...
#if !HZC_BUGFIX
  getLuma8x8Neighbour ( i, j, -1,  0, &block_a, uiPosition, uiBitSize, currMB, list_idx );
  getLuma8x8Neighbour ( i, j,  0, -1, &block_b, uiPosition, uiBitSize, currMB, list_idx );
  ...
  if ( block_a.available )
  {
    a = absm ( leftMB->mvd[list_idx][block_a.y][block_a.x][k] );
  }
  else
  {
    a = 0;
...
#else 
  if(currMB ->ui_MbBitSize==B8X8_IN_BIT||(( currMB ->ui_MbBitSize==B32X32_IN_BIT || currMB ->ui_MbBitSize==B16X16_IN_BIT)&& input->useSDIP))
#endif 
  {
...
#if WPM_ACE_OPT
    if (se->type == SE_WPM1) {
        biari_encode_symbol(eep_dp, (unsigned char)se->value1, img->currentSlice->syn_ctx->wpm_contexts);
    } else {
        biari_encode_symbol(eep_dp, (unsigned char)se->value1, img->currentSlice->syn_ctx->wpm_contexts + 1);
    }
#else
...
void writeCIPredMode ( codingUnit* currMB, SyntaxElement *se, EncodingEnvironmentPtr eep_dp, int uiPosition )
{
  ...
  else
  {
	...
	{
		if (act_sym > lmode)
			unary_bin_max_encode ( eep_dp, ( unsigned int ) ( act_sym - 2 ), ctx->c_intra_mode_contexts + 3, 0, 2 );
		else
			unary_bin_max_encode ( eep_dp, ( unsigned int ) ( act_sym - 1 ), ctx->c_intra_mode_contexts + 3, 0, 2 );
	}
	else
		unary_bin_max_encode ( eep_dp, ( unsigned int ) ( act_sym - 1 ), ctx->c_intra_mode_contexts + 3, 0, 3 );
#else 
	if( is_redundant )
	{
		if (act_sym > lmode)
...
void writeCbpBit ( int b8, int bit, int cbp, codingUnit* currMB, int inter, EncodingEnvironmentPtr eep_dp, int uiPosition )
{
  ...
  int currMB_hor=((currMB->cuType==P2NXN && uiBitSize>3||currMB->cuType==PHOR_UP||currMB->cuType==PHOR_DOWN)&&currMB->trans_size==1&&input->useNSQT==1 ||currMB->cuType==InNxNMB);
  int currMB_ver=((currMB->cuType==PNX2N && uiBitSize>3||currMB->cuType==PVER_LEFT||currMB->cuType==PVER_RIGHT)&&currMB->trans_size==1&&input->useNSQT==1 ||currMB->cuType==INxnNMB);
#endif 
  if (b8==4)
  {
    mb_x = mb_y = 0;
...
#if !TEXT_RD92_ALIGN
  getLuma8x8Neighbour ( mb_x, mb_y, -1,  0, &block_a, uiPosition, currMB->ui_MbBitSize, currMB, 0 );
  getLuma8x8Neighbour ( mb_x, mb_y,  0, -1, &block_b, uiPosition, currMB->ui_MbBitSize, currMB, 0 );
#else 
  mb_x = ( mb_x == 0 ) ? 0 : 1 ;
  mb_y = ( mb_y == 0 ) ? 0 : 1 ;
...
void writeCbpBit ( int b8, int bit, int cbp, codingUnit* currMB, int inter, EncodingEnvironmentPtr eep_dp, int uiPosition )
{
  ...
  if ( mb_x == 0 )
  {
    ...
    {
      a = ( ( ( ( currMB->mb_available_left )->cbp & ( 1 << ( 2 * ( mb_y / 2 ) + 1 ) ) ) == 0 ) ? 1 : 0 ); //VG-ADD
  ...
  biari_encode_symbol ( eep_dp, ( unsigned char ) bit,
    img->currentSlice->syn_ctx->cbp_contexts[0] + a + 2 * b );
}
#endif
/*!
****************************************************************************
...
#if INTRA_2N
  // intra cbp for a 16x16intra block only one bit for luma block
  if ( currMB->trans_size == 0 || currMB->cuType == I16MB )
  {
    b8 = 0;
    curr_cbp_idx = ( currMB->b8mode[0] == IBLOCK ? 0 : 1 );
    writeCbpBit ( b8, ( cbp & ( 0xF ) ) != 0, cbp, currMB, curr_cbp_idx, eep_dp, uiPosition );
  }
  else
#endif
    for ( b8 = 0; b8 < 4; b8++ )
    {
      ...
      writeCbpBit ( b8, cbp & ( 1 << b8 ), cbp, currMB, curr_cbp_idx, eep_dp, uiPosition );
    }
...
int estRunLevelRef( codingUnit* currMB,  int context )
{
  ...
  if (input->useNSQT && currMB->ui_MbBitSize > B8X8_IN_BIT && ((currMB->trans_size == 1) && (currMB->ui_MbBitSize == 6) && (IS_INTER(currMB)) && (currMB->cuType >= P2NXN && currMB->cuType <= PVER_RIGHT )))
  {
  ...
  {
	  Level = DCT_Level[k]= alllevel[k] ; // level
	  Run   = DCT_Run[k]=allrun[k] ; // run
	  DCT_Pairs++;
  }
...
#if INTRA_2N
  if ( ( currMB->trans_size == 0 || currMB->cuType == I16MB ) && currMB->ui_MbBitSize != MAX_CU_SIZE_IN_BIT  && se->context == LUMA_8x8)
  {
    numOfCoeff = ( 1 << currMB->ui_MbBitSize ) * ( 1 << currMB->ui_MbBitSize );
    bitSize += 1;
  }
  if (input->useNSQT  && currMB->ui_MbBitSize > B8X8_IN_BIT && ((currMB->trans_size == 1) && (currMB->ui_MbBitSize == 6) && (IS_INTER(currMB)) && (currMB->cuType >= P2NXN && currMB->cuType <= PVER_RIGHT ) && (se->context == LUMA_8x8)))
  {
    numOfCoeff = ( 1 << ( currMB->ui_MbBitSize - 2 ) ) * ( 1 << ( currMB->ui_MbBitSize - 2 ) );
    bitSize -=1;
  }
      ...
      if( DCT_CGFlag[ iCG ] )
      {
        ...
        for ( i = DCT_PairsInCG[ iCG ]; i >= 0; i--, pairs--, pairsInCG++ )
        {
...
#else
              if( (CGx == 0 && CGy > 0) || (bitSize == 0 && ( iCG == 0 ) && ( ctxmode == 1 )) )
#endif
              {
                yy=yy^xx;
                xx=yy^xx;
...
#if BBRY_CU8//???right?
#if !COEFF_BUGFIX
				  offset = (bitSize == 0) ? (ctxmode/2)*4 : ((CGx > 0 && CGy > 0) ? 0 : (  ( ctxmode/2 ) * 4 +  ( ( iCG == 0 ) ? 4 : 12 )) + 8);
#else 
				  offset = (bitSize == 0) ? (ctxmode/2)*4 : (((CGx > 0 && CGy > 0) ? 0 : (  ( ctxmode/2 ) * 4 +  ( ( iCG == 0 ) ? 4 : 12 ))) + 8);
#endif 
#else
                offset = ( bitSize > 0 ) ? ( 0 ) : ( (CGx > 0 && CGy > 0) ? 6 : ( ( iCG == 0 ) * ( ctxmode/2 + 1 )* 6 + 12 ) );
#endif
              }
              else
              {
...
#else
          offset = ((iCG == 0 && pos > 12) ? 0 : 3) + indiv + 5;
#endif
		  if (!isChroma)
		  {
			  offset += 3;
          ...
          if(15-pos>0)
          {
			...
			offset = ((iCG == 0) ? (pos+ctxpos == 14 ? 0 : (1 + moddiv)) : (se->context != LUMA_8x8 ? 2 : 3 + moddiv)) + (bitSize == 0 ? 0 : 3);
#else 
		   offset = ((iCG == 0) ? (pos+ctxpos == 14 ? 0 : (1 + moddiv)) : (se->context == LUMA_8x8 ? 2 : 3 + moddiv)) + (bitSize == 0 ? 0 : 3);
#endif 
#else
...
void writeSplitFlag_AEC ( codingUnit* currMB, SyntaxElement *se, EncodingEnvironmentPtr eep_dp, int uiPosition )
{
  ...
  if ( act_sym == 0 )
  {
    biari_encode_symbol ( eep_dp, 1, ctx->split_contexts + se->value2 );
  }
  else
  {
    biari_encode_symbol ( eep_dp, 0, ctx->split_contexts + se->value2 );
  }
#else 
...
int write_sao_type(SAOBlkParam* saoBlkParam,int uiPositionInPic)
{
	...
	if (saoBlkParam->typeIdc == SAO_TYPE_BO)
	{
		...
		if (AEC_writting)
		{
			fprintf(p_trace, "coded band = %d, second band = %d, delta band = %d\n", saoBlkParam->startBand,  saoBlkParam->startBand2,  saoBlkParam->deltaband);
		}
#endif
	}
	else
...
void write_sao_type_AEC(codingUnit* currMB, SyntaxElement* se, EncodingEnvironmentPtr eep_dp,int uiPositionInPic)
{
	...
	switch(se->value2)
	{
		case 0: length = NUM_SAO_EO_TYPES_LOG2; break;
		case 1: length = NUM_SAO_BO_CLASSES_LOG2; break;
		case 2: length = NUM_SAO_BO_CLASSES_LOG2 - 1; break;
	...
	if(se->value2 == 2)
	{
		...
		while ( 1 )
		{
			if ( temp >= ( unsigned int ) ( 1 << exp_golomb_order ) )
			{
				biari_encode_symbol_eq_prob ( eep_dp, 0 );
...
#if !M3544_CONTEXT_OPT
	biari_encode_symbol(eep_dp,symbol ,&(ctx->m_cALFLCU_Enable_SCModel[compIdx][ctx_idx]));
#else
